...
class AW_Rma_Model_Notify extends Mage_Core_Model_Abstract {
    ...
    protected function _notifyByStatus($rmaRequest, $commentText = NULL, $isFrontend = FALSE) {
        if ($this->_isGenerallyEnabled()) {
			...
			//added by Jack to show customer name in emailTemplate
			$customer_name = $rmaRequest->getCustomerName();
            ...
            if ($this->_isEnabledForCustomer() && $_status->getToCustomer()) {
                ...
                $emailTemplate = Mage::getModel('awrma/email_template')
                        ...
                        ->sendEmail($_sender, $rmaRequest->getCustomerEmail(), $rmaRequest->getCustomerName(), array(
            ...
            if ($this->_isEnabledForAdmin() && $_status->getToAdmin() && $isFrontend) {
                ...
                $emailTemplate = Mage::getModel('awrma/email_template')
                        ...
                        ->sendEmail($_sender, Mage::helper('awrma/config')->getDepartmentEmail($rmaRequest->getStoreId()), Mage::helper('awrma/config')->getDepartmentDisplayName($rmaRequest->getStoreId()), array(
					'customer_name'=>$customer_name,
                    'request' => $rmaRequest,
                    'store' => $_store,
                    'subject' => $_subject,
            ...
            if ($_status->getToChatbox()) {
                ...
                Mage::helper('awrma/comments')->postComment($rmaRequest->getId(), $emailTemplate->getProcessedTemplate(array(
                            'request' => $rmaRequest,
                            'store' => $_store,
                            'depname' => $_departmentDisplayName)), array('owner' => AW_Rma_Model_Source_Owner::SYSTEM), FALSE);
            }
