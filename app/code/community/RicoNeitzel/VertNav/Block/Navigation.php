...
class RicoNeitzel_VertNav_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    {
       //Mage::app()->cleanCache($this->getCacheKey());
    ...
    public function drawOpenCategoryItem( $category, $level = 0, array $levelClass = NULL )
    {
        ...
        if ($this->hideEmptyCategories() && !$n) return '';
    ...
    public function getStoreCategories()
    {
        ...
        switch( Mage::getStoreConfig( 'catalog/vertnav/vertnav_root' ) ) {
            ...
            default:
                ...
                if(Mage::registry( 'current_category' )->getBrandCategory() &&  Mage::registry( 'current_category' ) ) {
                	if( Mage::registry( 'current_category' )->getLevel() >= $fromLevel ) {
                		...
                		$parent = Mage::registry( 'current_category' )->getChildren();
                	}
        ...
        if(!Mage::registry( 'current_category' )->getBrandCategory()) $parent = Mage::app()->getStore()->getRootCategoryId();
        /**
    ...
    // V2014
    public function getNewStoreCategories()
    {
        try{
            ...
            if($shopCategory = $currentCategory->getShopCategory()) {
                ...
                $parentCategory = Mage::getModel('catalog/category')->load(intval($brand->getChildren()));
            ...
            $storeCategories = is_object($parentCategory) ? $this->_getCategoryCollection()->addFieldToFilter('parent_id', $parentCategory->getId()) : '';


            /* @var $category Mage_Catalog_Model_Category */
            $resourcedCategory = Mage::getResourceSingleton('catalog/category');
           /* if($storeCategories && !$currentCategory->getDynCategory()){
                foreach($storeCategories->getItems() as $key => $category ) {
                   // $resourcedCategory->getProductCount($category) < 1 && $storeCategories->removeItemByKey($key);
                }
            }*/

            return $storeCategories && $storeCategories->getSize() ? $storeCategories : $this->getStoreCategories();
        }catch (Exception $e){
            Mage::logException($e);
            return array();
        }
    ...
    protected function _getCategoryCollection()
    {
        ...
        if($brand = Mage::registry( 'current_category' )->getBrandCategory()) {
       $collection->addAttributeToSelect( 'url_key' )
	        ->addAttributeToSelect( 'name' )
	        ->addAttributeToSelect( 'all_children' )
	        ->addAttributeToSelect( 'include_in_menu')
	        ->addAttributeToFilter( 'is_active', 1 )
	        ->addAttributeToFilter('path', array('like' => '%/' . $brand->getId() . '/%'))
    ...
    {
        return Mage::getStoreConfigFlag( 'catalog/vertnav/hide_empty' );
    }
     ...
     * @return string
     */
    public function getPreSalePreOrderHtml($category)
    {
        //enable or disable
        $preSaleEnable = Mage::getSingleton('gri_presale/rule')->isEnabled();
        $preOrderEnable = Mage::getSingleton('gri_preorder/preorder')->isEnabled();

        //exist product
        $lis = '';
        /* @var $categoryHelper Gri_CatalogCustom_Helper_Category */
        $categoryHelper = Mage::helper('gri_catalogcustom/category');
        $preSalesCategory = $categoryHelper->getCategory($category, 'pre-sales');
        if ($preSalesCategory && $preSaleEnable && $this->getPreSaleProductIds($category)) {
            $lis .= '<li class="pre-sales"><a href="' . $preSalesCategory->getUrl() . '">' . $this->__('Pre Sales') . '</a></li> ';
        }
        $preOrderCategory = $categoryHelper->getCategory($category, 'pre-order');
        if ($preOrderCategory && $preOrderEnable && $this->getPreOrderProductIds($category)) {
            $lis .= '<li class="pre-order"><a href="' . $preOrderCategory->getUrl() . '">' . $this->__('Pre Order') . '</a></li> ';
        }
    ...
    /**
     * Check if pre-sale product exists
     *
     * @param Gri_CatalogCustom_Model_Category $category
     * @return boolean
     */
    public function getPreSaleProductIds($category = NULL)
    {
        return $this->getPreSaleHelper()->getProductIds($category);
    }

    /**
     * Check if pre-order product existed
     *
     * @param Gri_CatalogCustom_Model_Category $category
     * @return boolean
     */
    public function getPreOrderProductIds($category = NULL)
    {
        return $this->getPreOrderHelper()->getProductIds($category);
    }

    /**
     * @return Gri_Presale_Helper_Data
     */
    public function getPreSaleHelper()
    {
        return Mage::helper('gri_presale');
    }

    /**
     * @return Gri_Preorder_Helper_Data
     */
    public function getPreOrderHelper()
    {
        return Mage::helper('gri_preorder');
    }
}
