 ...
 */
class Gri_Cms_Model_Resource_Hierarchy_Node extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    protected function _getSliceChildren($object, $down = 0)
    {
        ...
        if (max($down, $this->_treeMaxDepth) > 0) {
            ...
            $select->where('scope_id = ', Mage::app()->getStore()->getId());
        }
        $select->order(array('level', $this->getMainTable() . '.sort_order'));
     ...
     * @param string $scope
     * @param int $scopeId
     * @param Gri_Cms_Model_Resource_Hierarchy_Node_Collection $collection
     * @return Gri_Cms_Model_Resource_Hierarchy_Node
     */
    public function copyTo($scope, $scopeId, $collection)
    {
        // Copy hierarchy
        /** @var $nodesModel Gri_Cms_Model_Hierarchy_Node */
        $nodesModel = Mage::getModel('gri_cms/hierarchy_node', array(
            'scope' =>  $scope,
            'scope_id' => $scopeId,
        ));
        ...
        foreach ($collection as $node) {
            if ($node->getLevel() == Gri_Cms_Model_Hierarchy_Node::NODE_LEVEL_FAKE) {
                continue;
            }
    ...
    public function deleteByScope($scope, $scopeId)
    {
        ...
        try {
            ...
            // Delete metadata
            $adapter->delete($this->getTable('gri_cms/hierarchy_metadata'), array(
                'node_id IN (?)' => $adapter
                    ->select()
    ...
    public function getIsInherited($scope, $scopeId)
    {
        ...
        $select = $adapter
            ...
            ->where('scope = ?', $scope)
            ->where('scope_id = ?', $scopeId)
            ->where('level = ?', Gri_Cms_Model_Hierarchy_Node::NODE_LEVEL_FAKE)
            ->limit(1);
    ...
    {
        if ($scope != Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_DEFAULT &&
