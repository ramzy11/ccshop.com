...
class Gri_GiftCardAccount_Model_System_Config_Backend_Pool extends Mage_Core_Model_Config_Data
{
    ...
    {
        if ($this->isValueChanged()) {
            Mage::getModel('gri_giftcardaccount/pool')->cleanupFree();
        }
        parent::_afterSave();
    }
    ...
    protected function _checkMaxLength()
    {
        ...
        if (isset($fields['code_split']['value'])) {
            ...
            if ($v > 0 && $v < $codeLen) {
                $sep = Mage::getModel('gri_giftcardaccount/pool')->getCodeSeparator();
                $len += (ceil($codeLen/$v) * strlen($sep))-1;
            }
        }

        if ($len > 255) {
            Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Maximum generated code length is 255. Please correct your settings.'));
        }
    }
}
