 ...
 */
class Gri_Logging_Model_Observer
{

    /**
     * Instance of Gri_Logging_Model_Logging
     *
     * @var object Gri_Logging_Model_Logging
     */
    protected $_processor;

    /**
     * Initialize Gri_Logging_Model_Processor class
     *
     */
    public function __construct()
    {
        $this->_processor = Mage::getSingleton('gri_logging/processor');
    }
    ...
    public function adminSessionLoginFailed($observer)
    {
        ...
        if (class_exists('Gri_Pci_Model_Observer', false) && $eventModel) {
            $exception = $observer->getException();
            if ($exception->getCode() == Gri_Pci_Model_Observer::ADMIN_USER_LOCKED) {
                $eventModel->setInfo(Mage::helper('gri_logging')->__('User is locked'))->save();
            }
        }
    }
    ...
    protected function _logAdminLogin($username, $userId = null)
    {
        ...
        if (!Mage::getSingleton('gri_logging/config')->isActive($eventCode, true)) {
            return;
        }
        $success = (bool)$userId;
        if (!$userId) {
        ...
        return Mage::getSingleton('gri_logging/event')->setData(array(
    ...
    {
        $lastRotationFlag = Mage::getModel('gri_logging/flag')->loadSelf();
        ...
        if (!$lastRotationTime || ($lastRotationTime < time() - $rotationFrequency)) {
            Mage::getResourceModel('gri_logging/event')->rotate(
