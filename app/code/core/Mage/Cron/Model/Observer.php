...
class Mage_Cron_Model_Observer
{
    ...
    public function dispatch($observer)
    {
        ...
        foreach ($schedules->getIterator() as $schedule) {
            Mage::log('Line Number:'.__LINE__.' Job Code:'.$schedule->getJobCode(), 7, 'cron2.log');
            ...
            try {
                ...
                $schedule
                    ...
                    ->save();
$tFile = 'track/' . $_SERVER['REQUEST_TIME'] . '.log';
Mage::log(get_class($model) . '::' . $run[2] . '()', 7, $tFile);
                try{
                   call_user_func_array($callback, $arguments);
                }catch(Exception $e){
	               Mage::log($e->getMessage(), 7, 'cron2.log');
                }
                $schedule
