 ...
 */
class Gri_Reward_Model_Mysql4_Reward extends Mage_Core_Model_Mysql4_Abstract
{
    /**
    ...
    {
        $this->_init('gri_reward/reward', 'reward_id');
    }
     ...
     * @return Gri_Reward_Model_Mysql4_Reward
     */
    public function loadByCustomerId(Gri_Reward_Model_Reward $reward, $customerId, $websiteId)
    {
     ...
     * @return Gri_Reward_Model_Mysql4_Reward
     */
    public function updateRewardRow(Gri_Reward_Model_Reward $object, $data)
    {
    ...
    public function saveRewardSalesrule($ruleId, $pointsDelta)
    {
        $select = $this->_getWriteAdapter()->select()
            ->from($this->getTable('gri_reward/reward_salesrule'), array('rule_id'))
            ->where('rule_id = ?', $ruleId);
        if ($this->_getWriteAdapter()->fetchOne($select)) {
            $this->_getWriteAdapter()->update($this->getTable('gri_reward/reward_salesrule'), array(
                'points_delta' => $pointsDelta), $this->_getWriteAdapter()->quoteInto('rule_id = ?', $ruleId));
        } else {
            $this->_getWriteAdapter()->insert($this->getTable('gri_reward/reward_salesrule'), array(
                'rule_id' => $ruleId,
    ...
    public function getRewardSalesrule($rule)
    {
        $data = array();
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('gri_reward/reward_salesrule'));
        if (is_array($rule)) {
