...
class Mage_Sales_Model_Resource_Quote_Item_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
     ...
     *
     * Migrated from ProxiBlue_GiftPromo_Model_Sales_Resource_Quote_Item_Collection::_countProductsRepeated()
     * @return array
     */
    protected function _countProductsRepeated() {
        $arr = array();
    ...
    protected function _assignProducts()
    {
        ...
        foreach ($this as $item) {
            ...
            if ($product) {
                // Migrated from ProxiBlue_GiftPromo_Model_Sales_Resource_Quote_Item_Collection::_assignProducts()
                // check if this product appears in the basket multiple times
                // if so, reload the poduct to fix the type
                $repeated = $this->_countProductsRepeated();
                if ($repeated[$item->getProductId()] > 1) {
                    $product = Mage::getModel('catalog/product')->setStoreId($this->getStoreId())->load($product->getId());
                }
