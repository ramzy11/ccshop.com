...
/**
 * @method array getExchangeItems() Get Exchange Items as OrderItemId => SKU
 * @method array getOrderItems() Get RMA Items OrderItemId => QTY
 */
class AW_Rma_Model_Entity extends Mage_Core_Model_Abstract {
    ...
    public function getTextId() {
        if ($this->getId() && $this->getData('text_id') === NULL) {
            ...
            $this->setData('text_id', $prefix . sprintf("%010d", $this->getId()));
        }
        return $this->getData('text_id');
    }
    ...
    protected function _afterLoad() {
        ...
        if (is_string($this->getExchangeItems()))
            $this->setExchangeItems((array)unserialize($this->getExchangeItems()));
    ...
    protected function _beforeSave() {
        ...
        if (!is_string($this->getExchangeItems()))
            $this->setExchangeItems(serialize($this->getExchangeItems()));
        if (!is_string($this->getPrintLabel()))
    ...
    /**
     * Get RMA Comments Collection
     * @return AW_Rma_Model_Mysql4_Entitycomments_Collection
     */
    public function getComments() {
        if (!$this->getData('comments')) {
            $comments = Mage::getModel('awrma/entitycomments')->getCollection()
                ->setEntityFilter($this->getId())
                ;
