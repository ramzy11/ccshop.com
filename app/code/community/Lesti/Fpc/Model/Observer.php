    ...
    public function httpResponseSendBefore($observer)
    {
        ...
        if ($fpc->isActive() &&
            ...
            if (in_array($fullActionName, $cacheableActions)) {
                ...
                $cache_time = null;                
				if(preg_match("/\/flashsale\//",Mage::helper('core/url')->getCurrentUrl()))
                {
                    $cache_time = 180;
                    Mage::log("set 3 minutes cache time for $key flashsale with url ".Mage::helper('core/url')->getCurrentUrl(),7,'gri-debug.log');
                }
                $fpc->save(serialize($object), $key, $this->_cache_tags, $cache_time);
