...
class Gri_Cms_Adminhtml_Cms_Page_RevisionController extends Gri_Cms_Adminhtml_Cms_PageController
{
    /**
     * Init actions
     *
    ...
    public function editAction()
    {
        ...
        if ($revisionId && !$revision->getId()) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('gri_cms')->__('Could not load specified revision.'));
        ...
        $this->_initAction()
            ->_addBreadcrumb(Mage::helper('gri_cms')->__('Edit Revision'),
                Mage::helper('gri_cms')->__('Edit Revision'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('gri_cms')->__('The revision has been saved.')
                );
    ...
    public function publishAction()
    {
        ...
        try {
            ...
            // display success message
            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('gri_cms')->__('The revision has been published.')
            );
    ...
    public function deleteAction()
    {
        ...
        if ($id = $this->getRequest()->getParam('revision_id')) {
            ...
            try {
                ...
                // display success message
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('gri_cms')->__('The revision has been deleted.')
                );
                $this->_redirect('*/cms_page_version/edit', array(
                        'page_id' => $revision->getPageId(),
                        'version_id' => $revision->getVersionId()
                ...
                Mage::logException($e);
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('gri_cms')->__('An error occurred while deleting the revision.')
                );
                $error = true;
            }
        ...
        // display error message
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('gri_cms')->__('Unable to find a revision to delete.')
        );
        // go to grid
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'save':
                return Mage::getSingleton('gri_cms/config')->canCurrentUserSaveRevision();
                break;
            case 'publish':
                return Mage::getSingleton('gri_cms/config')->canCurrentUserPublishRevision();
                break;
            case 'delete':
