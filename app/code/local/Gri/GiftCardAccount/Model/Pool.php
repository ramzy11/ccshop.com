...
class Gri_GiftCardAccount_Model_Pool extends Gri_GiftCardAccount_Model_Pool_Abstract
{
    ...
    const XML_CHARSET_NODE      = 'global/gri/giftcardaccount/charset/%s';
    const XML_CHARSET_SEPARATOR = 'global/gri/giftcardaccount/separator';

    const CODE_GENERATION_ATTEMPTS = 1000;

    protected function _construct()
    {
        $this->_init('gri_giftcardaccount/pool');
    }
        ...
        for ($i=0; $i<$size;$i++) {
            ...
            do {
                if ($attempt>=self::CODE_GENERATION_ATTEMPTS) {
                    Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Unable to create full code pool size. Please check settings and try again.'));
                }
     ...
     *
     * @return Gri_GiftCardAccount_Model_Pool
     */
