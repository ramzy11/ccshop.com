...
/**
 * Gri cms page observer
 *
 * @category    Gri
 * @package     Gri_Cms
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Gri_Cms_Model_Observer
{
    /**
     * Configuration model
    ...
    public function onMainTabPrepareForm($observer)
    {
         ...
         */
        /* @var $page Gri_Cms_Model_Page */
        $page = Mage::registry('cms_page');
        $revisionAvailable = false;
        if ($page) {

            $baseFieldset->addField('under_version_control', 'select', array(
                'label'     => Mage::helper('gri_cms')->__('Under Version Control'),
                'title'     => Mage::helper('gri_cms')->__('Under Version Control'),
            ...
            if ($page->getPublishedRevisionId() && $page->getUnderVersionControl()) {
                ...
                if ($revision->getId()) {
                    ...
                    $page->setPublishedRevisionLink(
                        Mage::helper('gri_cms')->__('%s; rev #%s', $versionLabel, $revisionNumber));

                    $baseFieldset->addField('published_revision_link', 'link', array(
                            'label' => Mage::helper('gri_cms')->__('Currently Published Revision'),
        ...
        if (!$revisionAvailable && $page->getId() && $page->getUnderVersionControl()) {
            ...
            $page->setPublishedRevisionStatus(Mage::helper('gri_cms')->__('Published Revision Unavailable'));
        }
    ...
    {
        /* @var $helper Gri_Cms_Helper_Hierarchy */
        $helper = Mage::helper('gri_cms/hierarchy');
        if (!$helper->isEnabled()) {
            return $this;
        }
        ...
        $node = Mage::getModel('gri_cms/hierarchy_node', array(
            'scope' => Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_STORE,
        ...
        if (!$node->getPageId()) {
            /* @var $child Gri_Cms_Model_Hierarchy_Node */
            $child = Mage::getModel('gri_cms/hierarchy_node', array(
                'scope' => $node->getScope(),
    ...
    public function cmsPageSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($page->getIsNewPage() || ($page->getUnderVersionControl()
            ...
            $version->setLabel($page->getTitle())
                ->setAccessLevel(Gri_Cms_Model_Page_Version::ACCESS_LEVEL_PUBLIC)
            ...
            if ($page->getUnderVersionControl()) {
                ...
                if ($revision instanceof Gri_Cms_Model_Page_Revision) {
                    $revision->publish();
                }
            }
        }

        if (!Mage::helper('gri_cms/hierarchy')->isEnabled()) {
            return $this;
        }

        // rebuild URL rewrites if page has changed for identifier
        if ($page->dataHasChangedFor('identifier')) {
            Mage::getSingleton('gri_cms/hierarchy_node')->updateRewriteUrls($page);
        }
         ...
         */
        Mage::getSingleton('gri_cms/hierarchy_node')->appendPageToNodes($page, $page->getAppendToNodes());
        ...
        foreach ($page->getNodesSortOrder() as $nodeId => $value) {
            Mage::getResourceSingleton('gri_cms/hierarchy_node')->updateSortOrder($nodeId, $value);
        }
    ...
    public function adminUserDeleteAfter(Varien_Event_Observer $observer)
    {
        ...
        $collection = $version->getCollection()
            ->addAccessLevelFilter(Gri_Cms_Model_Page_Version::ACCESS_LEVEL_PRIVATE)
            ->addUserIdFilter();
    ...
    public function deleteWebsite(Varien_Event_Observer $observer)
    {
        ...
        $nodeModel = Mage::getModel('gri_cms/hierarchy_node');
        $nodeModel->deleteByScope(Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_WEBSITE, $website->getId());
    ...
    {
        $storeScope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_STORE;
        $nodeModel = Mage::getModel('gri_cms/hierarchy_node');
        ...
        $widgets = $widgetModel->getResourceCollection()
                ...
                ->addFieldToFilter('instance_type','gri_cms/widget_node');
    ...
    public function modifyPageStatuses(Varien_Event_Observer $observer)
    {
        ...
        $statuses->setData(Mage_Cms_Model_Page::STATUS_ENABLED, Mage::helper('gri_cms')->__('Published'));
    ...
    public function cmsPageDeleteAfter(Varien_Event_Observer $observer)
    {
        ...
        Mage::getResourceSingleton('gri_cms/increment')
            ->cleanIncrementRecord(Gri_Cms_Model_Increment::TYPE_PAGE,
                $page->getId(),
                Gri_Cms_Model_Increment::LEVEL_VERSION);
     ...
     * @param Gri_Logging_Model_Event $eventModel
     * @return Gri_Logging_Model_Event|false
     */
    public function postDispatchCmsHierachyView($config, $eventModel)
    {
        return $eventModel->setInfo(Mage::helper('gri_cms')->__('Tree Viewed'));
    }
    ...
    public function addCmsToTopmenuItems(Varien_Event_Observer $observer)
    {
        ...
        $hierarchyModel = Mage::getModel('gri_cms/hierarchy_node', array(
            'scope' => Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_STORE,
            'scope_id' => Mage::app()->getStore()->getId(),
        ))->getHeritage();
     ...
     *
     * @param Gri_Cms_Model_Hierarchy_Node $cmsNode
     * @return bool
     */
