 ...
 * @package    Gri_Cms
 */
class Gri_Cms_Model_Mysql4_Hierarchy_Node extends Gri_Cms_Model_Resource_Hierarchy_Node
{
    /**
    ...
    {
        $this->_init('gri_cms/hierarchy_node', 'node_id');
        $this->_metadataTable = $this->getTable('gri_cms/hierarchy_metadata');
    }
     ...
     * @param string $field
     * @param mixed $value
     * @param Gri_Cms_Model_Hierarchy_Node $object
     * @return Varien_Db_Select
     */
    protected function _getLoadSelect($field, $value, $object)
    {
        $isFrontend = Mage::getDesign()->getArea() != 'adminhtml';
        /* @var $select Varien_Db_Select */
        $select = parent::_getLoadSelect($field, $value, $object);
        $isFrontend and $select->joinLeft(array('dp' => $this->getTable('cms/page')),
            $this->getMainTable() . '.page_id = dp.page_id', array()
        );

        0 and $isFrontend and $select->joinLeft(array('page_table' => $this->getTable('cms/page')),
                'page_table.identifier = dp.identifier', array(
                    'page_title' => 'title',
        ...
        $isFrontend or $select->joinLeft(array('page_table' => $this->getTable('cms/page')),
            $this->getMainTable() . '.identifier = page_table.identifier', array(
                'page_title' => 'title',
                'page_identifier' => 'identifier',
                'page_is_active' => 'is_active'
            )
        ...
        $isFrontend and $select->joinLeft(array('ps' => $this->getTable('cms/page_store')),
            'ps.page_id = page_table.page_id', array()
        );

      /*  $select->joinLeft(array('metadata_table' => $this->_metadataTable),
     ...
     * @return Gri_Cms_Model_Mysql4_Hierarchy_Node
     */
    protected function _applyParamFilters($select)
    {
        ...
        $select->where($this->getMainTable() . '.scope_id = ? ', Mage::app()->getStore()->getId());
     ...
     *
     * @param bool $flag
     * @return Gri_Cms_Model_Hierarchy_Node
     */
     ...
     * @param Gri_Cms_Model_Hierarchy_Node $node The parent node
     * @param string $type
     * @return array|bool
     */
    public function getMetaNodeDataByType($node, $type)
    {
        ...
        if ($read) {
            ...
            switch ($type) {
                case Gri_Cms_Model_Hierarchy_Node::META_NODE_TYPE_CHAPTER:
                case Gri_Cms_Model_Hierarchy_Node::META_NODE_TYPE_SECTION:
                    $fieldName = 'meta_chapter';
                    if ($type == Gri_Cms_Model_Hierarchy_Node::META_NODE_TYPE_SECTION) {
                ...
                case Gri_Cms_Model_Hierarchy_Node::META_NODE_TYPE_FIRST:
                    $found = true;
                ...
                case Gri_Cms_Model_Hierarchy_Node::META_NODE_TYPE_PREVIOUS:
                ...
                case Gri_Cms_Model_Hierarchy_Node::META_NODE_TYPE_NEXT:
                    $found = true;
                    $addParentNodeCondition = true;
                    $select->where($this->getMainTable() . '.sort_order>?', $node->getSortOrder());
                    $select->order($this->getMainTable() . '.sort_order ASC');
...
//    }
     ...
     * @return array
     */
    public function getParentNodes($object)
    {
        $parentIds = explode('/', $object->getXpath());
        Mage::log(var_export($object->getXpath(), true),7,'node.log');
        array_pop($parentIds); //remove self node
        $select = $this->_getLoadSelectWithoutWhere()
            ->where($this->getMainTable().'.node_id IN (?)', $parentIds)
            ->order($this->getMainTable().'.level ASC');

        Mage::getDesign()->getArea() == 'adminhtml' or $select->where($this->getMainTable().'.scope_id = ? OR '.$this->getMainTable() . '.page_id IS NULL ', Mage::app()->getStore()->getId());
        Mage::log($select->__toString(),7,'node.log');
        $nodes = $select->query()->fetchAll();
        return $nodes;
    }
     ...
     * @return array
     */
    public function getTreeMetaData(Gri_Cms_Model_Hierarchy_Node $object) {
        $read = $this->_getReadAdapter();
        $select = $read->select();
        $xpath = explode('/', $object->getXpath());
