 ...
 *
 */
class Gri_Reward_Model_System_Config_Backend_Balance extends Mage_Core_Model_Config_Data
{
    /**
     * Check if max_points_balance >= than min_points_balance
     * (max allowed to RP to gain is more than minimum to redeem)
     *
     * @return Gri_Reward_Model_System_Config_Backend_Balance
     */
    protected function _beforeSave()
    {
        ...
        if ($this->getFieldsetDataValue('min_points_balance') < 0) {
            Mage::throwException(Mage::helper('gri_reward')->__('"Minimum Reward Points Balance" should be positive number or empty.'));
        }
        if ($this->getFieldsetDataValue('max_points_balance') < 0) {
            Mage::throwException(Mage::helper('gri_reward')->__('"Cap Reward Points Balance" should be positive number or empty.'));
        }
        if ($this->getFieldsetDataValue('max_points_balance') &&
            ...
            Mage::throwException(Mage::helper('gri_reward')->__('"Minimum Reward Points Balance" should be less or equal to "Cap Reward Points Balance".'));
        }
        return $this;
    }
}
