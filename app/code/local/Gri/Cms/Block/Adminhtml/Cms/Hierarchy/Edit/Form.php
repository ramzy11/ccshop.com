 ...
 */
class Gri_Cms_Block_Adminhtml_Cms_Hierarchy_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    /**
    ...
    {
        parent::__construct();
        $this->setTemplate('gri/cms/hierarchy/edit.phtml');
     ...
     *
     * @return Gri_Cms_Block_Adminhtml_Cms_Hierarchy_Edit_Form
     */
    protected function _prepareForm()
    {
        ...
        $fieldset   = $form->addFieldset('node_properties_fieldset', array(
            'legend'    => Mage::helper('gri_cms')->__('Page Properties')
        ));
        ...
        $fieldset->addField('node_label', 'text', array(
            'name'      => 'label',
            'label'     => Mage::helper('gri_cms')->__('Title'),
            'required'  => true,
        ...
        $fieldset->addField('node_identifier', 'text', array(
            'name'      => 'identifier',
            'label'     => Mage::helper('gri_cms')->__('URL Key'),
            'required'  => true,
        ...
        $fieldset->addField('node_preview', 'link', array(
            ...
            'href'      => '#',
            'value'     => Mage::helper('gri_cms')->__('No preview available'),
        ));
         ...
         */
        if (Mage::helper('gri_cms/hierarchy')->isMetadataEnabled()) {
            $fieldset   = $form->addFieldset('metadata_fieldset', array(
                'legend'    => Mage::helper('gri_cms')->__('Render Metadata in HTML Head')
            ));


            $fieldset->addField('meta_first_last', 'select', array(
                'label'     => Mage::helper('gri_cms')->__('First'),
                'title'     => Mage::helper('gri_cms')->__('First'),
            ...
            $fieldset->addField('meta_next_previous', 'select', array(
                'label'     => Mage::helper('gri_cms')->__('Next/Previous'),
                'title'     => Mage::helper('gri_cms')->__('Next/Previous'),
            ...
            $fieldset->addField('meta_cs_enabled', 'select', array(
                'label'     => Mage::helper('gri_cms')->__('Enable Chapter/Section'),
                'title'     => Mage::helper('gri_cms')->__('Enable Chapter/Section'),
                'name'      => 'meta_cs_enabled',
                'values'    => $yesNoOptions,
                'onchange'  => 'hierarchyNodes.nodeChanged()',
                'container_id' => 'field_meta_cs_enabled',
                'note'      => Mage::helper('gri_cms')->__('Enables Chapter/Section functionality for this node, its sub-nodes and pages'),
                'tabindex'  => '45'
            ));

            $fieldset->addField('meta_chapter_section', 'select', array(
                'label'     => Mage::helper('gri_cms')->__('Chapter/Section'),
                'title'     => Mage::helper('gri_cms')->__('Chapter/Section'),
                'name'      => 'meta_chapter_section',
                'values'    => Mage::getSingleton('gri_cms/source_hierarchy_menu_chapter')->toOptionArray(),
                'onchange'  => 'hierarchyNodes.nodeChanged()',
                'container_id' => 'field_meta_chapter_section',
                'note'      => Mage::helper('gri_cms')->__('Defines this node as Chapter/Section'),
                'tabindex'  => '50'
            ));
        }
        ...
        $pagerFieldset   = $form->addFieldset('pager_fieldset', array(
            'legend'    => Mage::helper('gri_cms')->__('Pagination Options for Nested Pages')
        ));

        $pagerFieldset->addField('pager_visibility', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('Enable Pagination'),
            'name'      => 'pager_visibility',
            'values'    => Mage::getSingleton('gri_cms/source_hierarchy_visibility')->toOptionArray(),
            'value'     => Gri_Cms_Helper_Hierarchy::METADATA_VISIBILITY_PARENT,
            'onchange'  => "hierarchyNodes.metadataChanged('pager_visibility', 'pager_fieldset')",
            'tabindex'  => '70'
        ));
        $pagerFieldset->addField('pager_frame', 'text', array(
            'name'      => 'pager_frame',
            'label'     => Mage::helper('gri_cms')->__('Frame'),
            'class'     => 'validate-digits',
            'onchange'  => 'hierarchyNodes.nodeChanged()',
            'container_id' => 'field_pager_frame',
            'note'      => Mage::helper('gri_cms')->__('How many Links to display at once'),
            'tabindex'  => '80'
        ));
        $pagerFieldset->addField('pager_jump', 'text', array(
            'name'      => 'pager_jump',
            'label'     => Mage::helper('gri_cms')->__('Frame Skip'),
            'class'     => 'validate-digits',
            'onchange'  => 'hierarchyNodes.nodeChanged()',
            'container_id' => 'field_pager_jump',
            'note'      => Mage::helper('gri_cms')->__('If the Current Frame Position does not cover Utmost Pages, will render Link to Current Position plus/minus this Value'),
            'tabindex'  => '90'
        ));
        ...
        $menuFieldset   = $form->addFieldset('menu_fieldset', array(
            'legend'    => Mage::helper('gri_cms')->__('Page Navigation Menu Options')
        ));

        $menuFieldset->addField('menu_excluded', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('Exclude from Navigation Menu'),
            'name'      => 'menu_excluded',
            'values'    => $yesNoOptions,
            'onchange'   => "hierarchyNodes.nodeChanged()",
        ...
        $menuFieldset->addField('menu_visibility', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('Show in Navigation Menu'),
        ...
        $menuFieldset->addField('menu_layout', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('Menu Layout'),
            'name'      => 'menu_layout',
            'values'    => Mage::getSingleton('gri_cms/source_hierarchy_menu_layout')->toOptionArray(true),
            'onchange'   => "hierarchyNodes.nodeChanged()",
        ...
        $menuBriefOptions = array(
            array('value' => 1, 'label' => Mage::helper('gri_cms')->__('Only Children')),
            array('value' => 0, 'label' => Mage::helper('gri_cms')->__('Neighbours and Children')),
        );
        $menuFieldset->addField('menu_brief', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('Menu Detalization'),
            'name'      => 'menu_brief',
        ...
        $menuFieldset->addField('menu_levels_down', 'text', array(
            ...
            'label'     => Mage::helper('gri_cms')->__('Maximal Depth'),
            'class'     => 'validate-digits',
            'onchange'  => 'hierarchyNodes.nodeChanged()',
            'container_id' => 'field_menu_levels_down',
            'note'      => Mage::helper('gri_cms')->__('Node Levels to Include'),
            'tabindex'  => '130'
        ));
        $menuFieldset->addField('menu_ordered', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('List Type'),
            'title'     => Mage::helper('gri_cms')->__('List Type'),
            'name'      => 'menu_ordered',
            'values'    => Mage::getSingleton('gri_cms/source_hierarchy_menu_listtype')->toOptionArray(),
        ...
        $menuFieldset->addField('menu_list_type', 'select', array(
            'label'     => Mage::helper('gri_cms')->__('List Style'),
            'title'     => Mage::helper('gri_cms')->__('List Style'),
            'name'      => 'menu_list_type',
            'values'    => Mage::getSingleton('gri_cms/source_hierarchy_menu_listmode')->toOptionArray(),
            'onchange'  => 'hierarchyNodes.nodeChanged()',
        ...
        $menuFieldset   = $form->addFieldset('top_menu_fieldset', array(
            'legend'    => Mage::helper('gri_cms')->__('Main Navigation Menu Options')
        ));
    ...
    public function getPagePropertiesButtons()
    {
        ...
        $buttons[] = $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
            ...
            'label'     => Mage::helper('gri_cms')->__('Remove From Tree'),
            'onclick'   => 'hierarchyNodes.deleteNodePage()',
            'class'     => 'delete'
        ))->toHtml();
        $buttons[] = $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
            'id'        => 'cancel_node_button',
            'label'     => Mage::helper('gri_cms')->__('Cancel'),
            'onclick'   => 'hierarchyNodes.cancelNodePage()',
            'class'     => 'delete'
        ))->toHtml();
        $buttons[] = $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
            'id'        => 'save_node_button',
            'label'     => Mage::helper('gri_cms')->__('Save'),
    ...
    public function getTreeButtonsHtml()
    {
        return $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
            'id'        => 'new_node_button',
            'label'     => Mage::helper('gri_cms')->__('Add Node...'),
    ...
    {
        /** @var $nodeModel Gri_Cms_Model_Hierarchy_Node */
     ...
     *
     * @param Gri_Cms_Model_Hierarchy_Node $node
    ...
    {
        return Mage::helper('gri_cms')->__('Add To Tree');
    }
    ...
    {
        return Mage::helper('gri_cms')->__('Update');
    }
    ...
    {
        return Mage::helper('gri_cms')->__('Node Properties');
    }
    ...
    {
        return Mage::helper('gri_cms')->__('Page Properties');
    }
    ...
    protected function _getStore()
    {
        ...
        } elseif ($this->getCurrentScope() == Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_WEBSITE) {
    ...
    {
        $listModes = Mage::getSingleton('gri_cms/source_hierarchy_menu_listmode')->toOptionArray();
        $result = array();
    ...
    {
        return Mage::helper('gri_cms')->__('Page lock expires in 60 seconds. Save changes to avoid possible data loss.');
    }
     ...
     *
     * @deprecated since 1.12.0.0
     * @return Gri_Cms_Model_Hierarchy_Lock
     */
    protected function _getLockModel()
    {
        return Mage::getSingleton('gri_cms/hierarchy_lock');
    }
}
