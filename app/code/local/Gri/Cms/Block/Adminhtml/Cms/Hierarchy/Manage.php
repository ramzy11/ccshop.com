 ...
 */
class Gri_Cms_Block_Adminhtml_Cms_Hierarchy_Manage extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($currentStore) {
            ...
            $excludeScopes = array(
                Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_STORE . $storeId
            );
        } elseif ($currentWebsite) {
            $websiteId = Mage::app()->getWebsite($currentWebsite)->getId();
            $excludeScopes = array(
                Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_WEBSITE . $websiteId
            );
        }
        ...
        $form->addField('scopes', 'multiselect', array(
            'name'      => 'scopes[]',
            'class'     => 'manage-select',
            'title'     => Mage::helper('gri_cms')->__('Manage Hierarchies'),
    ...
    protected function _prepareOptions($all = false, $excludeScopes)
    {
        ...
        foreach ($storeStructure as $website) {
            $value = Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_WEBSITE . $website['value'];
            if (isset($website['children'])) {
                ...
                foreach ($website['children'] as $store) {
                    if (isset($store['children']) && !in_array($store['value'], $excludeScopes)) {
                        ...
                        foreach ($store['children'] as $storeView) {
                            $storeView['value'] = Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_STORE
                                                  . $storeView['value'];
                ...
                $website['value'] = Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_STORE
