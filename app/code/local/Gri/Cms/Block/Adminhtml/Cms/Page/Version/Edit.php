 ...
 */
class Gri_Cms_Block_Adminhtml_Cms_Page_Version_Edit
    extends Mage_Adminhtml_Block_Widget_Form_Container
{
    protected $_objectId   = 'version_id';
    protected $_blockGroup = 'gri_cms';
    ...
    public function __construct()
    {
        ...
        if ($config->canCurrentUserSaveVersion()) {
            $this->_addButton('new', array(
                    'label'     => Mage::helper('gri_cms')->__('Save as New Version'),
                    'onclick'   => "editForm.submit('" . $this->getNewUrl() . "');",
                    'class'     => 'new',
                ));

            $this->_addButton('new_revision', array(
                    'label'     => Mage::helper('gri_cms')->__('New Revision...'),
        ...
        if (($isOwner || $isPublisher) && $config->canCurrentUserSaveVersion()) {
            $this->_addButton('saveandcontinue', array(
                'label'     => Mage::helper('gri_cms')->__('Save and Continue Edit'),
    ...
    public function getHeaderText()
    {
        ...
        if (!$versionLabel) {
            $versionLabel = Mage::helper('gri_cms')->__('N/A');
        }

        return Mage::helper('gri_cms')->__("Edit Page '%s' Version '%s'", $title, $versionLabel);
    }
