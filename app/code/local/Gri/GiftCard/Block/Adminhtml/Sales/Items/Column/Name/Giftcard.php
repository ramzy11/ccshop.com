...
class Gri_GiftCard_Block_Adminhtml_Sales_Items_Column_Name_Giftcard extends Mage_Adminhtml_Block_Sales_Items_Column_Name
{
    /**
    ...
    protected function _getGiftcardOptions()
    {
        ...
        if ($type = $this->getItem()->getProductOptionByCode('giftcard_type')) {
            switch ($type) {
                case Gri_GiftCard_Model_Giftcard::TYPE_VIRTUAL:
                    $type = Mage::helper('gri_giftcard')->__('Virtual');
                    break;
                case Gri_GiftCard_Model_Giftcard::TYPE_PHYSICAL:
                    $type = Mage::helper('gri_giftcard')->__('Physical');
                    break;
                case Gri_GiftCard_Model_Giftcard::TYPE_COMBINED:
                    $type = Mage::helper('gri_giftcard')->__('Combined');
                    break;
            }

            $result[] = array(
                'label'=>Mage::helper('gri_giftcard')->__('Gift Card Type'),
                'value'=>$type,
            );
        }
        ...
        if ($value = $this->_prepareCustomOption('giftcard_sender_name')) {
            ...
            $result[] = array(
                'label'=>Mage::helper('gri_giftcard')->__('Gift Card Sender'),
                'value'=>$value,
                'custom_view'=>true,
            );
        }
        if ($value = $this->_prepareCustomOption('giftcard_recipient_name')) {
            ...
            $result[] = array(
                'label'=>Mage::helper('gri_giftcard')->__('Gift Card Recipient'),
                'value'=>$value,
                'custom_view'=>true,
            );
        }
        if ($value = $this->_prepareCustomOption('giftcard_message')) {
            $result[] = array(
                'label'=>Mage::helper('gri_giftcard')->__('Gift Card Message'),
                'value'=>$value,
            );
        }
        ...
        if ($value = $this->_prepareCustomOption('giftcard_lifetime')) {
            $result[] = array(
                'label'=>Mage::helper('gri_giftcard')->__('Gift Card Lifetime'),
                'value'=>sprintf('%s days', $value),
            );
        }

        $yes = Mage::helper('gri_giftcard')->__('Yes');
        $no = Mage::helper('gri_giftcard')->__('No');
        if ($value = $this->_prepareCustomOption('giftcard_is_redeemable')) {
            $result[] = array(
                'label'=>Mage::helper('gri_giftcard')->__('Gift Card Is Redeemable'),
                'value'=>($value ? $yes : $no),
            );
        }
        ...
        if (is_array($codes)) {
            foreach ($codes as &$code) {
                if ($code === null) {
                    $code = Mage::helper('gri_giftcard')->__('Unable to create.');
                }
            }
        } else {
            $codes = array();
        ...
        for ($i = $createdCodes; $i < $totalCodes; $i++) {
            $codes[] = Mage::helper('gri_giftcard')->__('N/A');
        }

        $result[] = array(
            'label'=>Mage::helper('gri_giftcard')->__('Gift Card Accounts'),
