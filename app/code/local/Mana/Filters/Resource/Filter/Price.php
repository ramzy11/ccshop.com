...
class Mana_Filters_Resource_Filter_Price extends Mage_Catalog_Model_Resource_Eav_Mysql4_Layer_Filter_Price {
    ...
    public function getMaxPrice($filter)
    {
        ...
        /* @var $currentCategory ProxiBlue_DynCatProd_Model_Category */
        $currentCategory = Mage::getSingleton('catalog/layer')->getCurrentCategory();

        $isDynCategory = $currentCategory->getIsDynCategory();
...
//        $this->getCount($filter, 1);
    ...
    public function getMinPrice($filter)
    {
        ...
        $maxPriceExpr = new Zend_Db_Expr("MIN({$table}.max_price {$additional}) AS m_min_price");
        $dynCategory = Mage::getSingleton('catalog/layer')->getCurrentCategory()->getDynCategory();
        if(!$dynCategory){
            // MANA BEGIN: make sure no filter is applied
            Mage::helper('mana_filters')->resetProductCollectionWhereClause($select);
        }
        ...
        // MANA END
        $select->columns(array($maxPriceExpr))->order('m_min_price DESC');
