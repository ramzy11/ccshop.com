 ...
 */
class Gri_Reward_Adminhtml_Reward_RateController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Check if module functionality enabled
     *
     * @return Gri_Reward_Adminhtml_Reward_RateController
     */
    public function preDispatch()
    {
        parent::preDispatch();
        if (!Mage::helper('gri_reward')->isEnabled() && $this->getRequest()->getActionName() != 'noroute') {
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('gri_reward')->__('Customers'),
                Mage::helper('gri_reward')->__('Customers'))
            ->_addBreadcrumb(Mage::helper('gri_reward')->__('Manage Reward Exchange Rates'),
                Mage::helper('gri_reward')->__('Manage Reward Exchange Rates'));
        return $this;
    }
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            try {
                $rate->save();
                $this->_getSession()->addSuccess(Mage::helper('gri_reward')->__('The rate has been saved.'));
            } catch (Exception $e) {
                Mage::logException($e);
    ...
    public function deleteAction()
    {
        ...
        if ($rate->getId()) {
            try {
                $rate->delete();
                $this->_getSession()->addSuccess(Mage::helper('gri_reward')->__('The rate has been deleted.'));
            } catch (Exception $e) {
    ...
    public function validateAction()
    {
        ...
        } elseif ($post['direction'] == Gri_Reward_Model_Reward_Rate::RATE_EXCHANGE_DIRECTION_TO_CURRENCY
        ...
        } elseif ($post['direction'] == Gri_Reward_Model_Reward_Rate::RATE_EXCHANGE_DIRECTION_TO_POINTS
