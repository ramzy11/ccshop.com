 ...
 * @package    Gri_Cms
 */
class Gri_Cms_Adminhtml_Cms_HierarchyController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Current Scope
     *
     * @var string
     */
    protected $_scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_DEFAULT;
     ...
     * @var int
     */
    protected $_scopeId = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_DEFAULT_ID;
     ...
     *
     * @return Gri_Cms_HierarchyController
     */
    ...
    protected function _initScope()
    {
        ...
        if (!is_null($this->_website)) {
            $this->_scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_WEBSITE;
        ...
        if (!is_null($this->_store)) {
            $this->_scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_STORE;
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('gri_cms')->__('CMS'),
                Mage::helper('gri_cms')->__('CMS'))
            ->_addBreadcrumb(Mage::helper('gri_cms')->__('CMS Page Trees'),
                Mage::helper('gri_cms')->__('CMS Page Trees'));
        return $this;
    }
    ...
    {
        $scopeId = false;
        $scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_DEFAULT;
        if (0 === strpos($value, Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_WEBSITE)) {
            $scopeId = (int)str_replace(Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_WEBSITE, '', $value);
            $scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_WEBSITE;
        } elseif (0 === strpos($value, Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_STORE)) {
            $scopeId = (int)str_replace(Gri_Cms_Helper_Hierarchy::SCOPE_PREFIX_STORE, '', $value);
            $scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_STORE;
        }
        if (!$scopeId || $scopeId == Mage_Catalog_Model_Abstract::DEFAULT_STORE_ID) {
            $scopeId = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_DEFAULT_ID;
            $scope = Gri_Cms_Model_Hierarchy_Node::NODE_SCOPE_DEFAULT;
        }
        ...
        if (empty($scopes) || ($this->getRequest()->isPost() && !is_array($scopes))
            ...
            try {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('gri_cms')->__('There has been an error deleting hierarchy.')
                );
            }
        }
    ...
    public function copyAction()
    {
        ...
        if ($this->getRequest()->isPost() && is_array($scopes) && !empty($scopes)) {
            /** @var $nodeModel Gri_Cms_Model_Hierarchy_Node */
            $nodeModel = Mage::getModel('gri_cms/hierarchy_node', array(
                'scope' =>  $this->_scope,
            ...
            try {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('gri_cms')->__('There has been an error copying hierarchy.')
                );
            }
        }
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                $hasError = false;
                $this->_getSession()->addSuccess(
                    Mage::helper('gri_cms')->__('The hierarchy has been saved.')
                );
            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                $this->_getSession()->addException($e,
                    Mage::helper('gri_cms')->__('There has been an error saving hierarchy.')
                );
            }
