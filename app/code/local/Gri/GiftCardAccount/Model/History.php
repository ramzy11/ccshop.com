...
class Gri_GiftCardAccount_Model_History extends Mage_Core_Model_Abstract
{
    ...
    {
        $this->_init('gri_giftcardaccount/history');
    }

    public function getActionNamesArray()
    {
        return array(
            self::ACTION_CREATED  => Mage::helper('gri_giftcardaccount')->__('Created'),
            self::ACTION_UPDATED  => Mage::helper('gri_giftcardaccount')->__('Updated'),
            self::ACTION_SENT     => Mage::helper('gri_giftcardaccount')->__('Sent'),
            self::ACTION_USED     => Mage::helper('gri_giftcardaccount')->__('Used'),
            self::ACTION_REDEEMED => Mage::helper('gri_giftcardaccount')->__('Redeemed'),
            self::ACTION_EXPIRED  => Mage::helper('gri_giftcardaccount')->__('Expired'),
        );
    }
    ...
    protected function _getCreatedAdditionalInfo()
    {
        if ($this->getGiftcardaccount()->getOrder()) {
            ...
            return Mage::helper('gri_giftcardaccount')->__('Order #%s.', $orderId);
        } else if ($user = Mage::getSingleton('admin/session')->getUser()) {
            $username = $user->getUsername();
            if ($username) {
                return Mage::helper('gri_giftcardaccount')->__('By admin: %s.', $username);
            }
        }else if($this->getGiftcardaccount()->getProduct()){
            $productId = $this->getGiftcardaccount()->getProduct()->getId();
            return Mage::helper('gri_giftcardaccount')->__('Product of giftcard #%s.', $productId);
        }
    ...
    protected function _getSentAdditionalInfo()
    {
        ...
        if ($sender) {
            return Mage::helper('gri_giftcardaccount')->__('Recipient: %s. By admin: %s.', $recipient, $sender);
        } else {
            return Mage::helper('gri_giftcardaccount')->__('Recipient: %s.', $recipient);
        }
    }
    ...
    protected function _getRedeemedAdditionalInfo()
    {
        if ($customerId = $this->getGiftcardaccount()->getCustomerId()) {
            return Mage::helper('gri_giftcardaccount')->__('Customer #%s.', $customerId);
        }
        return '';
    }
    ...
    protected function _beforeSave()
    {
        if (!$this->hasGiftcardaccount()) {
            Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Please assign gift card account.'));
        }
        ...
        switch ($this->getGiftcardaccount()->getHistoryAction()) {
            ...
            break;
            default:
                Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Unknown history action.'));
            break;
        }
