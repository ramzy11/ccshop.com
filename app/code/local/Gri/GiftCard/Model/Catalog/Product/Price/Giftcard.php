...
class Gri_GiftCard_Model_Catalog_Product_Price_Giftcard extends Mage_Catalog_Model_Product_Type_Price
{
    /**
     * Cached amounts
     * @var array
    ...
    public function getRewardPoints($qty=null, $product)
    {
        $rewardPoints = $product->getPrice();
        $rewardPoints or $rewardPoints = $this->getMinAmount($product);
        if ($product->hasCustomOptions()) {
            $customOption = $product->getCustomOption('giftcard_amount');
            if ($customOption) {
                $rewardPoints += $customOption->getValue();
            }
        }

        $rewardPoints = $this->_applyOptionsPrice($product, $qty, $rewardPoints);
        // Convert currency to points
        $rateToCurrency = Mage::getSingleton('gri_reward/reward')->getRateToCurrency();
        $rewardPoints = intval($rewardPoints * $rateToCurrency->getPoints() / $rateToCurrency->getCurrencyAmount());

        $product->setData('reward_points', $rewardPoints);
        return max(0, $product->getData('reward_points'));
    }
