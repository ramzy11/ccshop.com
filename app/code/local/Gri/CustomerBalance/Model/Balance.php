 ...
 *
 */
class Gri_CustomerBalance_Model_Balance extends Mage_Core_Model_Abstract
{
    /**
    ...
    {
        $this->_init('gri_customerbalance/balance');
    }
     ...
     *
     * @return Gri_CustomerBalance_Model_Balance
    ...
    public function loadByCustomer()
    {
        ...
        if ($this->hasWebsiteId()) {
            ...
            if (Mage::app()->getStore()->isAdmin()) {
                Mage::throwException(Mage::helper('gri_customerbalance')->__('Website ID must be set.'));
            }
    ...
    public function setNotifyByEmail($shouldNotify, $storeId = null)
    {
        ...
        if ($shouldNotify) {
            if (null === $storeId) {
                Mage::throwException(Mage::helper('gri_customerbalance')->__('Please set store ID as well.'));
            }
    ...
    protected function _beforeSave()
    {
        ...
        if (!$this->getId()) {
            $this->loadByCustomer();
            if (!$this->getId()) {
                $this->setHistoryAction(Gri_CustomerBalance_Model_Balance_History::ACTION_CREATED);
            }
        }
        if (!$this->hasHistoryAction()) {
            $this->setHistoryAction(Gri_CustomerBalance_Model_Balance_History::ACTION_UPDATED);
        }
        ...
        if ($this->getNotifyByEmail() && !$this->hasStoreId()) {
            Mage::throwException(Mage::helper('gri_customerbalance')->__('In order to send email notification, the Store ID must be set.'));
        }
    ...
    protected function _afterSave()
    {
        ...
        if (abs($this->getAmountDelta())) {
            $history = Mage::getModel('gri_customerbalance/balance_history')
    ...
    protected function _ensureCustomer()
    {
        ...
        if (!$this->getCustomerId()) {
            Mage::throwException(Mage::helper('gri_customerbalance')->__('Customer ID must be specified.'));
        }
        if (!$this->getCustomer()) {
            $this->setCustomer(Mage::getModel('customer/customer')->load($this->getCustomerId()));
        }
        if (!$this->getCustomer()->getId()) {
            Mage::throwException(Mage::helper('gri_customerbalance')->__('Customer is not set or does not exist.'));
        }
    }
