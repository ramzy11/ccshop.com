...
class Gri_GiftCardAccount_Model_Giftcardaccount extends Mage_Core_Model_Abstract
{
    ...
    protected $_eventPrefix = 'gri_giftcardaccount';
    ...
    protected $_defaultPoolModelClass = 'gri_giftcardaccount/pool';
    ...
    protected function _beforeSave()
    {
        ...
        if (is_numeric($this->getLifetime()) && $this->getLifetime() > 0) {
            ...
            if ($this->getDateExpires()) {
                ...
                if ($expirationDate < $currentDate) {
                    Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Expiration date cannot be in the past.'));
                }
            } else {
        ...
        if (!$this->getId() && !$this->hasHistoryAction()) {
            $this->setHistoryAction(Gri_GiftCardAccount_Model_History::ACTION_CREATED);
        }

        if (!$this->hasHistoryAction() && $this->getOrigData('balance') != $this->getBalance()) {
            $this->setHistoryAction(Gri_GiftCardAccount_Model_History::ACTION_UPDATED)
                ->setBalanceDelta($this->getBalance() - $this->getOrigData('balance'));
        }
        if ($this->getBalance() < 0) {
            Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Balance cannot be less than zero.'));
        }
    }
    ...
    protected function _afterSave()
    {
        if ($this->getIsNew()) {
            $this->getPoolModel()
                ->setId($this->getCode())
                ->setStatus(Gri_GiftCardAccount_Model_Pool_Abstract::STATUS_USED)
                ->save();
    ...
    public function addToCart($saveQuote = true, $quote = null)
    {
        ...
        if ($this->isValid(true, true, $website)) {
            $cards = Mage::helper('gri_giftcardaccount')->getCards($quote);
            if (!$cards) {
                $cards = array();
            } else {
                foreach ($cards as $one) {
                    if ($one['i'] == $this->getId()) {
                        Mage::throwException(Mage::helper('gri_giftcardaccount')->__('This gift card account is already in the quote.'));
                    }
                }
            }
            $cards[] = array(
            ...
            Mage::helper('gri_giftcardaccount')->setCards($quote, $cards);
    ...
    {
        if (!$this->getId()) {
            $this->_throwException(Mage::helper('gri_giftcardaccount')->__('Wrong gift card account code: "%s".', $this->_requestedCode));
        }
        if (is_null($quote)) {
            $quote = $this->_getCheckoutSession()->getQuote();
        }
        ...
        $this->_throwException(Mage::helper('gri_giftcardaccount')->__('This gift card account wasn\'t found in the quote.'));
    }
    ...
    public function isValid($expirationCheck = true, $statusCheck = true, $websiteCheck = false, $balanceCheck = true)
    {
        if (!$this->getId()) {
            $this->_throwException(
                Mage::helper('gri_giftcardaccount')->__('Wrong gift card account ID. Requested code: "%s"', $this->_requestedCode)
            );
        }
        ...
        if ($websiteCheck) {
            ...
            if ($this->getWebsiteId() != $website) {
                $this->_throwException(
                    Mage::helper('gri_giftcardaccount')->__('Wrong gift card account website: %s.', $this->getWebsiteId())
                );
            }
        }
        ...
        if ($statusCheck && ($this->getStatus() != self::STATUS_ENABLED)) {
            $this->_throwException(
                Mage::helper('gri_giftcardaccount')->__('Gift card account %s is not enabled.', $this->getId())
            );
        }

        if ($expirationCheck && $this->isExpired()) {
            $this->_throwException(
                Mage::helper('gri_giftcardaccount')->__('Gift card account %s is expired.', $this->getId())
            );
        }
        ...
        if ($balanceCheck) {
            if ($this->getBalance() <= 0) {
                $this->_throwException(
                    Mage::helper('gri_giftcardaccount')->__('Gift card account %s balance does not have funds.', $this->getId())
                );
            }
            if ($balanceCheck !== true && is_numeric($balanceCheck)) {
                if ($this->getBalance() < $balanceCheck) {
                    $this->_throwException(
                        Mage::helper('gri_giftcardaccount')->__('Gift card account %s balance is less than amount to be charged.', $this->getId())
                    );
                }
            }
        }
    ...
    public function charge($amount)
    {
        if ($this->isValid(false, false, false, $amount)) {
            $this->setBalanceDelta(-$amount)
                ...
                ->setHistoryAction(Gri_GiftCardAccount_Model_History::ACTION_USED);
        }
    ...
    public function revert($amount)
    {
        ...
        if ($amount > 0 && $this->isValid(true, true, false, false)) {
            $this->setBalanceDelta($amount)
                ->setBalance($this->getBalance() + $amount)
                ->setHistoryAction(Gri_GiftCardAccount_Model_History::ACTION_UPDATED);
        }
    ...
    public function getStatesAsOptionList()
    {
        ...
        $result[self::STATE_AVAILABLE] = Mage::helper('gri_giftcardaccount')->__('Available');
        $result[self::STATE_USED]      = Mage::helper('gri_giftcardaccount')->__('Used');
        $result[self::STATE_REDEEMED]  = Mage::helper('gri_giftcardaccount')->__('Redeemed');
        $result[self::STATE_EXPIRED]   = Mage::helper('gri_giftcardaccount')->__('Expired');
     ...
     *
     * @return Gri_GiftCardAccount_Model_Pool_Abstract
     */
    ...
    public function redeem($customerId = null)
    {
        if ($this->isValid(true, true, true, true)) {
            ...
            if (!$customerId) {
                Mage::throwException(Mage::helper('gri_giftcardaccount')->__('Invalid customer ID supplied.'));
            }

            $additionalInfo = Mage::helper('gri_giftcardaccount')
                ->__('Gift Card Redeemed: %s. For customer #%s.', $this->getCode(), $customerId);

            $balance = Mage::getModel('gri_customerbalance/balance')
            ...
            $this->setBalanceDelta(-$this->getBalance())
                ->setHistoryAction(Gri_GiftCardAccount_Model_History::ACTION_REDEEMED)
                ->setBalance(0)
                ->setCustomerId($customerId)
                ->save();
        }
    ...
    public function sendEmail()
    {
        ...
        if ($email->getSentSuccess()) {
            $this->setEmailSent(true)
                ->setHistoryAction(Gri_GiftCardAccount_Model_History::ACTION_SENT)
                ->save();
        }
    }
    ...
    protected function _throwException($realMessage, $fakeMessage = '')
    {
        ...
        Mage::logException($e);
        if (!$fakeMessage) {
            $fakeMessage = Mage::helper('gri_giftcardaccount')->__('Wrong gift card code.');
        }
