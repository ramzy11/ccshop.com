...
$installer->run("
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms_hierarchy')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms/hierarchy_node')}` (
  ...
  CONSTRAINT `FK_ENTERPRISE_CMS_HIERARCHY_NODE_TREE` FOREIGN KEY (`tree_id`) REFERENCES `{$installer->getTable('gri_cms_hierarchy')}` (`tree_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
");
$installer->endSetup();
