...
class AW_Rma_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    {
        $this->_productModel === NULL and $this->_productModel = $productModel = Mage::getModel('catalog/product');
     ...
     */
    protected function _getStockSingleton()
    {
    ...
    /**
     * @param string $sku
     * @return int|bool Product ID for the SKU
     */
    public function checkSku($sku)
    {
        return $this->_getProductModel()->getIdBySku($sku);
    }
    ...
    public function subtractInventory($data)
    {
        foreach ($data as $productId => $qty) {
            try {
                $this->_getStockSingleton()->registerProductsSale(array($productId => array('qty' => $qty)));
            }
            catch (Exception $e) {
                continue;
            }
    ...
    public function getRequestTypes()
    {
        ...
        foreach($collection->toOptionArray() as $option ) {
            $key = $option['value'];
            $typeOptions[$key] = strtolower($option['label']);
        }
