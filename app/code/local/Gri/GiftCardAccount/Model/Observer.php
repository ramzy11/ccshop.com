...
class Gri_GiftCardAccount_Model_Observer
{
    /**
     ...
     * @return Gri_GiftCardAccount_Model_Observer
     */
    public function processOrderPlace(Varien_Event_Observer $observer)
    {
        $order = $observer->getEvent()->getOrder();
        $cards = Mage::helper('gri_giftcardaccount')->getCards($order);
        if (is_array($cards)) {
            foreach ($cards as &$card) {
                ...
                Mage::dispatchEvent('gri_giftcardaccount_charge', $args);
                $card['authorized'] = $card['ba'];
            }

            $cards = Mage::helper('gri_giftcardaccount')->setCards($order, $cards);
        }
     ...
     * used for event: gri_giftcardaccount_charge_by_code
     *
     * @param Varien_Event_Observer $observer
     * @return Gri_GiftCardAccount_Model_Observer
     */
    public function chargeByCode(Varien_Event_Observer $observer)
    {
        ...
        Mage::getModel('gri_giftcardaccount/giftcardaccount')
            ->loadByCode($id)
            ->charge($amount)
     ...
     * used for event: gri_giftcardaccount_charge
     *
     * @param Varien_Event_Observer $observer
     * @return Gri_GiftCardAccount_Model_Observer
     */
     ...
     * used for event: gri_giftcardaccount_create
     *
     * @param Varien_Event_Observer $observer
     * @return Gri_GiftCardAccount_Model_Observer
     */
    public function create(Varien_Event_Observer $observer)
    {
        ...
        $model = Mage::getModel('gri_giftcardaccount/giftcardaccount')
            ->setStatus(Gri_GiftCardAccount_Model_Giftcardaccount::STATUS_ENABLED)
     ...
     * used for event: gri_giftcardaccount_save_after
     *
     * @param Varien_Event_Observer $observer
     * @return Gri_GiftCardAccount_Model_Observer
     */
    public function giftcardaccountSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($gca->hasHistoryAction()) {
            Mage::getModel('gri_giftcardaccount/history')
        ...
        if ($paypalCart) {
            ...
            if ($value > 0.0001) {
                $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, $value,
                    Mage::helper('gri_giftcardaccount')->__('Gift Card (%s)', Mage::app()->getStore()->convertPrice($value, true, false))
                );
            }
        }
    }
    ...
    {
        $giftCard = Mage::getModel('gri_giftcardaccount/giftcardaccount')->load($id);
