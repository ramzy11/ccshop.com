...
$installer->run("
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms/page_version')}` (
  `version_id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `label` VARCHAR(255) DEFAULT NULL,
  `access_level` ENUM('".Gri_Cms_Model_Page_Version::ACCESS_LEVEL_PRIVATE."',
                '".Gri_Cms_Model_Page_Version::ACCESS_LEVEL_PROTECTED."',
                '".Gri_Cms_Model_Page_Version::ACCESS_LEVEL_PUBLIC."') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms/page_revision')}` (
  ...
  CONSTRAINT `FK_CMS_REVISION_VERSION_ID` FOREIGN KEY (`version_id`) REFERENCES `{$installer->getTable('gri_cms/page_version')}` (`version_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms/increment')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms/hierarchy_metadata')}` (
  ...
  PRIMARY KEY (`node_id`),
  CONSTRAINT `FK_ENTERPRISE_CMS_HIERARCHY_METADATA_NODE` FOREIGN KEY (`node_id`) REFERENCES `{$installer->getTable('gri_cms/hierarchy_node')}` (`node_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms/hierarchy_node')}` (
  ...
  CONSTRAINT `FK_ENTERPRISE_CMS_HIERARCHY_NODE_PARENT_NODE` FOREIGN KEY (`parent_node_id`) REFERENCES `{$installer->getTable('gri_cms/hierarchy_node')}` (`node_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms_widget_instance_page')}` (
  ...
  CONSTRAINT `FK_WIDGET_INSTANCE_ID` FOREIGN KEY (`instance_id`) REFERENCES `{$installer->getTable('gri_cms_widget_instance')}` (`instance_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_cms_widget_instance_page_layout')}` (
    ...
    CONSTRAINT `FK_WIDGET_INSTANCE_PAGE_ID` FOREIGN KEY (`page_id`) REFERENCES `{$installer->getTable('gri_cms_widget_instance_page')}` (`page_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8
");
...
$select->from(array('p' =>  $installer->getTable('cms/page'), array('*')))
    ->joinLeft(array('v' =>  $installer->getTable('gri_cms/page_version')), 'v.page_id = p.page_id', array())
...
try {
    ...
    while($page = $resource->fetch(Zend_Db::FETCH_ASSOC)) {
        $installer->getConnection()->insert($installer->getTable('gri_cms/increment'), array(
            'type' => 0,
            'node' => $page['page_id'],
            'level' => 0,
            'last_id' => 1
        ...
        $installer->getConnection()->insert($installer->getTable('gri_cms/page_version'), array(
            'version_number' => 1,
        ...
        $versionId = $installer->getConnection()->lastInsertId($installer->getTable('gri_cms/page_version'), 'version_id');
        ...
        $_data['meta_keywords'] = '';
        $_data['meta_description'] = '';


        $installer->getConnection()->insert($installer->getTable('gri_cms/page_revision'), $_data);
    }
...
SET published_revision_id = (SELECT revision_id FROM
        ' . $installer->getTable('gri_cms/page_version') . ' as v, ' . $installer->getTable('gri_cms/page_revision') . ' as r
