...
//            $this->_items = array();
    ...
    protected function _getItemsData()
    {
		...
		//if($attribute->getAttributeCode() == 'size') return $this->_getSizeItemsData();
        if ($data === null) {
            ...
            if(in_array($attribute->getAttributeCode(), array('color_filter_1','size_filter_1'))) {
            	...
            	$secondOptions = $secondAttribute->getFrontend()->getSelectOptions();
            	$attributeOption = array();
            	foreach($attribute->getSource()->getAllOptions() as $sub_option) {
            		$attributeOption[$sub_option['label']] =  $sub_option['value'];
            	}
            	foreach($secondOptions as $option) {
            		...
            		if (Mage::helper('core/string')->strlen($option['value'])) {
            			// Check filter type
            			if ($this->_getIsFilterableAttribute($secondOptions) == self::OPTIONS_ONLY_WITH_RESULTS) {

            				if (!empty($optionsCount[$option['value']]) || in_array($option['value'], $selectedOptionIds)) {
            					$data[] = array(
            						'label' => $option['label'],
            						'value' => $attributeOption[$option['label']],
            						'count' => isset($optionsCount[$option['value']]) ? $optionsCount[$option['value']] : 0,
            						// MANA BEGIN: mark each selected item now in memory so we could later mark it
            						// visually in markup
            						'm_selected' => in_array($attributeOption[$option['label']], $selectedOptionIds),
            						// MANA END
            					);
            				}
            			}
    ...
    protected function _getSizeItemsData() {
    	...
    	$data = $this->getLayer()->getAggregator()->getCacheData($key);
    	if ($data === null) {
    		$options = Mage::helper('gri_catalogcustom')->getAttributeOptionsWithAdminLabel($attribute);
    		$sizeValues = Mage::helper('gri_catalogcustom')->getSizeValues($attribute);
    		$originalOptionsCount = $this->_getResource()->getCount($this);
    		$universalSizes = array();
    		foreach(Mage::getModel('gri_catalogcustom/sizemap')->getCollection() as $size) $universalSizes[$size->getData('admin_size')] = $size->getData('universal_size');
    		$optionsCount = array();
    		// for no map size
    		foreach($originalOptionsCount as $k => $v) {
    			if(isset($universalSizes[$k])) $k = $universalSizes[$k];
    			$optionsCount[$k] = $v;
    		}
    		$data = array();
    		foreach ($sizeValues as $k => $v) {
    			/*
    			if (is_array($option['value'])) {
    				continue;
    			}
    			*/
    			if (Mage::helper('core/string')->strlen($v)) {
    				if ($this->_getIsFilterableAttribute($attribute) == self::OPTIONS_ONLY_WITH_RESULTS) {
    					if (isset($optionsCount[$k])) {
    						$data[] = array(
    							'label' => $k,
    							'value' => $v,
    							'count' => isset($optionsCount[$k]) ? $optionsCount[$k] : 0,
    							// MANA BEGIN: mark each selected item now in memory so we could later mark it
    							// visually in markup
    							'm_selected' => array_intersect(Mage::helper('mana_core')->sanitizeNumber($v, array('_')), $selectedOptionIds),
    							// MANA END
    						);
    					}
    				}
