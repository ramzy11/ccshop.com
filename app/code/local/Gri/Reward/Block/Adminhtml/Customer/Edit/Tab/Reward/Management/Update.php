 ...
 */
class Gri_Reward_Block_Adminhtml_Customer_Edit_Tab_Reward_Management_Update
    extends Mage_Adminhtml_Block_Widget_Form
{
     ...
     *
     * @return Gri_Reward_Block_Adminhtml_Customer_Edit_Tab_Reward_Management_Update
     */
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('update_fieldset', array(
            'legend' => Mage::helper('gri_reward')->__('Update Reward Points Balance')
        ));

        if (!Mage::app()->isSingleStoreMode()) {
            $fieldset->addField('store', 'select', array(
                'name'  => 'store_id',
                'title' => Mage::helper('gri_reward')->__('Store'),
                'label' => Mage::helper('gri_reward')->__('Store'),
        ...
        $fieldset->addField('points_delta', 'text', array(
            'name'  => 'points_delta',
            'title' => Mage::helper('gri_reward')->__('Update Points'),
            'label' => Mage::helper('gri_reward')->__('Update Points'),
            'note'  => Mage::helper('gri_reward')->__('Enter a negative number to subtract from balance.')
        ));

        $fieldset->addField('comment', 'text', array(
            'name'  => 'comment',
            'title' => Mage::helper('gri_reward')->__('Comment'),
            'label' => Mage::helper('gri_reward')->__('Comment')
        ));

        $fieldset = $form->addFieldset('notification_fieldset', array(
            'legend' => Mage::helper('gri_reward')->__('Reward Points Notifications')
        ));

        $fieldset->addField('update_notification', 'checkbox', array(
            'name'    => 'reward_update_notification',
            'label'   => Mage::helper('gri_reward')->__('Subscribe for balance updates'),
        ...
        $fieldset->addField('warning_notification', 'checkbox', array(
            ...
            'label'   => Mage::helper('gri_reward')->__('Subscribe for points expiration notifications'),
