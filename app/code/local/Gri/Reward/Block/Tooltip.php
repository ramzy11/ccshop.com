 ...
 */
class Gri_Reward_Block_Tooltip extends Mage_Core_Block_Template
{
    /**
     * Reward instance
     *
     ...
     *
     * @var Gri_Reward_Model_Action_Abstract
     */
    ...
    public function initRewardType($action)
    {
        if ($action) {
            ...
            $this->_rewardInstance = Mage::getSingleton('gri_reward/reward')
    ...
    public function getRewardAmount($amount = null, $asCurrency = false)
    {
        ...
        return Mage::helper('gri_reward')->formatAmount($amount, $asCurrency);
    }

    public function renderLearnMoreLink($format = '<a href="%1$s">%2$s</a>', $anchorText = null)
    {
        $anchorText = null === $anchorText ? Mage::helper('gri_reward')->__('Learn more...') : $anchorText;
    ...
    protected function _prepareTemplateData()
    {
        if ($this->_actionInstance) {
            $this->addData(array(
                ...
                'landing_page_url' => Mage::helper('gri_reward')->getLandingPageUrl(),
            ));
            ...
            if ($this->_rewardInstance->getId()) {
                ...
                if ($this->hasIsGuestNote() && !$this->hasGuestNote()) {
                    $this->setGuestNote(Mage::helper('gri_reward')->__('Applies only to registered customers, may vary when logged in.'));
                }
            }
        }
    }
