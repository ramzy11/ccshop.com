...
$installer->run("
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_reward')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_reward_history')}` (
  ...
  CONSTRAINT `FK_REWARD_HISTORY_REWARD_ID` FOREIGN KEY (`reward_id`) REFERENCES `{$installer->getTable('gri_reward')}` (`reward_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('gri_reward_rate')}` (
