...
class Amasty_Rules_Model_Observer
{
    ...
    protected function _initRule($rule, $address, $quote)
    {
        ...
        $prices = $this->_getSortedCartPrices($rule, $address);
    ...
    protected function _initRuleXY($rule, $address, $quote)
    {
        ...
        // 1 cent fix
        $centCorrect = $rule->getSimpleAction() == Amasty_Rules_Helper_Data::TYPE_XY_PERCENT ? round($part*$realQty,2) - $discount : 0;
        $baseCentCorrect = $rule->getSimpleAction() == Amasty_Rules_Helper_Data::TYPE_XY_PERCENT ? round($basePart*$realQty,2) - $baseDiscount : 0;
     ...
     * @return array
     */
    protected function _getSortedCartPrices($rule, $address)
    {
        $prices = array();
        foreach ($this->_getAllItems($address) as $item) {
