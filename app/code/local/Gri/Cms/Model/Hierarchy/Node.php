 ...
 *
 * @method Gri_Cms_Model_Resource_Hierarchy_Node _getResource()
 * @method Gri_Cms_Model_Resource_Hierarchy_Node getResource()
 * @method int getParentNodeId()
 * @method Gri_Cms_Model_Hierarchy_Node setParentNodeId(int $value)
 * @method int getPageId()
 * @method Gri_Cms_Model_Hierarchy_Node setPageId(int $value)
 * @method Gri_Cms_Model_Hierarchy_Node setIdentifier(string $value)
 * @method Gri_Cms_Model_Hierarchy_Node setLabel(string $value)
 * @method int getLevel()
 * @method Gri_Cms_Model_Hierarchy_Node setLevel(int $value)
 * @method int getSortOrder()
 * @method Gri_Cms_Model_Hierarchy_Node setSortOrder(int $value)
 * @method string getRequestUrl()
 * @method Gri_Cms_Model_Hierarchy_Node setRequestUrl(string $value)
 * @method string getXpath()
 * @method Gri_Cms_Model_Hierarchy_Node setXpath(string $value)
 *
 * @category    Gri
 * @package     Gri_Cms
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Gri_Cms_Model_Hierarchy_Node extends Mage_Core_Model_Abstract
{
    /**
    ...
    public function getNodesData()
    {
        ...
        foreach ($collection as $item) {
            ...
            $nodes[] = Mage::helper('gri_cms/hierarchy')->copyMetaData($item->getData(), $node);
        }
     ...
     *
     * @return Gri_Cms_Model_Resource_Hierarchy_Node_Collection
     */
    ...
    {
        $this->_init('gri_cms/hierarchy_node');
    }
        ...
        foreach ($data as $v) {
            ...
            foreach ($required as $field) {
                if (!array_key_exists($field, $v)) {
                    Mage::throwException(
                        Mage::helper('gri_cms')->__('Invalid node data.')
                    );
                }
            }
            ...
            $nodes[$parentNodeId][$v['node_id']] = Mage::helper('gri_cms/hierarchy')
     ...
     * @param string $scope
     * @param int $scopeId
     * @return Gri_Cms_Model_Resource_Hierarchy_Node
     */
    ...
    public function getMetaNodeByType($type)
    {
        if (!isset($this->_metaNodes[$type])) {
            $model = Mage::getModel('gri_cms/hierarchy_node')
    ...
    public function getTreeSlice($up = 0, $down = 0)
    {
        ...
        $blankModel = Mage::getModel('gri_cms/hierarchy_node');
        foreach ($data as $parentId => $children) {
            foreach ($children as $childId => $child) {
                $newModel = clone $blankModel;
                $child['scope_id'] == Mage::app()->getStore()->getId() && $data[$parentId][$childId] = $newModel->setData($child);
            }
        }
        return $data;
    }
    ...
    {
        $nodes = $this->_getResource()->getParentNodes($this);
        $blankModel = Mage::getModel('gri_cms/hierarchy_node');
        foreach ($nodes as $id => $node) {
            $newModel = clone $blankModel;
            $nodes[$id] = $newModel->setData($node);
        }
        return $nodes;
    }
    ...
    {
        $values = array(
            Gri_Cms_Helper_Hierarchy::METADATA_VISIBILITY_YES,
            Gri_Cms_Helper_Hierarchy::METADATA_VISIBILITY_NO);
    ...
    public function getMenuLayout()
    {
        ...
        $layout = Mage::getSingleton('gri_cms/hierarchy_config')->getContextMenuLayout($layoutCode);
    ...
    public function getHeritage()
    {
        if ($this->getIsInherited()) {
            ...
            $parentScopeNode = Mage::getModel('gri_cms/hierarchy_node', array(
                'scope' =>  $parentScope[0],
