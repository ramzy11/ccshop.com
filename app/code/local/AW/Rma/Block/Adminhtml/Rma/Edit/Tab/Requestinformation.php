...
class AW_Rma_Block_Adminhtml_Rma_Edit_Tab_Requestinformation extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $details->addField('order_id_url', 'link', array(
            'name' => 'order_id_url',
        ...
        if ($formData->getCustomerId()) {
            ...
            $details->addField('customer_id_url', 'link', array(
        ...
        if ($formData->getExternalLink())
            $details->addField('external_link', 'link', array(
                'name' => 'external_link',
        ...
        // Request type should not be changed
        $formData->getId() and $typeOptions['disabled'] = 'disabled';
        $requestoptions->addField('request_type', 'select', $typeOptions);
    ...
    public function getStatusOptions($storeId = 0)
    {
        return $this->_getStatusOptions(new Varien_Object(array('store_id' => $storeId)));
    }

    public function getTypesOptions($storeId = 0)
    {
        return $this->_getTypesOptions(new Varien_Object(array('store_id' => $storeId)));
    }
}
