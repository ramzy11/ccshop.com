    ...
    Hamper.prototype = {
            ...
            if (item.length) {
                ...
                if (item.hasClass('configurable')) {
                    var optionContainer = newItem.find('.product-option'),
                        ...
                        defaultFixedOption = item.find('.options li.hover').length ?
                        ...
                        spConfigAttributes = spConfig.attributes;;
                    ...
                    if( item.find('.options li').length ) {
                        defaultFixedOption = defaultFixedOption.attr('option').split('-');
                        optionContainer.append('<input type="hidden" name="hamper_option[' + option + '][' + pickIndex +
                          '][' + selection + '][' + spConfig.attributeIdsToChange[0] + ']" value="' + defaultFixedOption[1] + '"/>');

                        // Output other options
                        var spConfigAttributes = JSON.toArray(spConfigAttributes);
                        spConfigAttributes.sort(function(a, b) { return b.id - a.id} );
                        _.each(spConfigAttributes, function (attribute, v) {
                            attribute = v.id;
                            ...
                            select.append('<option allowed="" value=""' + (selected ? '' : ' selected="selected"') + '>' + Translator.translate('Select %s').replace('%s', v.label) + '</option>');
                            _.each(v.options, function (i, o) {
                                ...
                                if ( _(allowedProducts).filter(o.products).length && v.id != spConfig.attributeIdsToChange[0] ) {
                                    select.append('<option value="' + o.id + '"' + (selected == o.id? ' selected="selected"' : '') + ' allowed="' + o.products.join(',') + '">' + o.label + '</option>');
                                    (selected == o.id) && (targetProduct = _(allowedProducts).filter(o.products)[0]);
                                }
                            });
                            ...
                            select.selectBox(); // active selctBox
                    });
                ...
                }else if(item.hasClass('simple')) {
                    newItem.append('<input type="hidden" class="target-product" name="target_product[' + pickIndex + ']" value="' + product + '" />');
                }
        ...
        updateTargetProduct: function (e) {
            var select = _(e.delegateTarget),
                ...
                targetParentProduct = field.next('input[type=hidden]'),
                allowedProducts = JSON.toArray(spConfig.allowedProducts),
                colorAllowedProducts,
                intersection,
                sizeSelect,
                finalAttributesAllowedProducts;

            var liIndex =  select.attr('id').split('_')[1],
                attributeId = select.attr('id').split('_')[2],
                otherAttributeLabel;

            if( attributeId == spConfig.attributeIdsToChange[0] ) {  // 181
                ...
                _.each(spConfig.attributes, function (i, v) {
                    if( v.id != spConfig.attributeIdsToChange[0] ) {
                        sizeSelect = _('#attr_'+liIndex +'_'+ v.id).empty();
                        return false;
                    }
                })

                // color allowedProducts
                colorAllowedProducts = option.attr('allowed').split(',');
                _.each(spConfig.attributes, function (i, v) {
                     if( v.id  && v.id != spConfig.attributeIdsToChange[0]) {
                         _.each(v.options, function (index, option) {
                             intersection = arrayIntersection(option.products, colorAllowedProducts);
                             if( intersection.length ) {
                                 sizeSelect.append('<option value="' + option.id + '" allowed="' + option.products.join(',') + '">' + option.label + '</option>');
                             }
                         })

                         sizeSelect.prepend('<option value="" allowed="" selected="selected">' + Translator.translate('Select %s').replace('%s', v.label) + '</option>');
                     }
                })

                sizeSelect && sizeSelect.selectBox('refresh');
            }

            // update target product input
            var attributesAllowedProducts = [], ii=0;
            _.each(spConfig.attributes, function (i, v) {

                var optionValue = jQuery('#attr_'+liIndex+'_'+ v.id).val();
                attributesAllowedProducts[ii++] =  jQuery('#attr_'+liIndex+'_'+ v.id+' option[value='+optionValue+']').attr('allowed') ?
                                 jQuery('#attr_'+liIndex+'_'+ v.id+' option[value='+optionValue+']').attr('allowed').split(',') : [];
            })

            finalAttributesAllowedProducts = attributesAllowedProducts[0];
            if( attributesAllowedProducts.length > 1 ) {
                finalAttributesAllowedProducts = arrayIntersection(attributesAllowedProducts[0], attributesAllowedProducts[1]);
            }

            field.val(parseInt(finalAttributesAllowedProducts[0]));
