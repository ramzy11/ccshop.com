...
class Gri_GiftCardAccount_Model_Total_Quote_GiftCardAccount extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    /**
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if ($baseUsed) {
            $quoteCards = $this->_sortGiftCards(Mage::helper('gri_giftcardaccount')->getCards($address->getQuote()));
        ...
        Mage::helper('gri_giftcardaccount')->setCards($address, $usedAddressCards);
        $address->setUsedGiftCards($address->getGiftCards());
        Mage::helper('gri_giftcardaccount')->setCards($address, $addressCards);
    ...
    protected function _collectQuoteGiftCards($quote)
    {
        if (!$quote->getGiftCardsTotalCollected()) {
            ...
            foreach ($cards as $k=>&$card) {
                $model = Mage::getModel('gri_giftcardaccount/giftcardaccount')->load($card['i']);
     ...
     *
     * @param Mage_Sales_Model_Quote_Address $address
     * @return Gri_GiftCardAccount_Model_Total_Quote_GiftCardAccount
     */
    public function fetch(Mage_Sales_Model_Quote_Address $address)
    {
        if ($address->getQuote()->isVirtual()) {
            $giftCards = Mage::helper('gri_giftcardaccount')->getCards($address->getQuote()->getBillingAddress());
        } else {
            $giftCards = Mage::helper('gri_giftcardaccount')->getCards($address);
        }
        $address->addTotal(array(
            'code'=>$this->getCode(),
            'title'=>Mage::helper('gri_giftcardaccount')->__('Gift Cards'),
