...
class Gri_GiftCardAccount_Block_Adminhtml_Giftcardaccount_Edit_Tab_Send extends Mage_Adminhtml_Block_Widget_Form
{
    public function initForm()
    {
        ...
        $fieldset = $form->addFieldset('base_fieldset',
            array('legend'=>Mage::helper('gri_giftcardaccount')->__('Send Gift Card'))
        );
...
/*
        ...
        $fieldset->addField('email_template', 'select', array(
            'label'     => Mage::helper('gri_giftcardaccount')->__('Email Template'),
            'title'     => Mage::helper('gri_giftcardaccount')->__('Email Template'),
            'name'      => 'email_template',
...
*/
        ...
        $fieldset->addField('recipient_email', 'text', array(
            'label'     => Mage::helper('gri_giftcardaccount')->__('Recipient Email'),
            'title'     => Mage::helper('gri_giftcardaccount')->__('Recipient Email'),
            'class'     => 'validate-email',
            'name'      => 'recipient_email',
        ));

        $fieldset->addField('recipient_name', 'text', array(
            'label'     => Mage::helper('gri_giftcardaccount')->__('Recipient Name'),
            'title'     => Mage::helper('gri_giftcardaccount')->__('Recipient Name'),
            'name'      => 'recipient_name',
        ));

        $fieldset->addField('store_id', 'select', array(
            'name'     => 'recipient_store',
            'label'    => Mage::helper('gri_customerbalance')->__('Send Email from the Following Store View'),
            'title'    => Mage::helper('gri_customerbalance')->__('Send Email from the Following Store View'),
    ...
    protected function _getStoreIdScript()
    {
        ...
        $result .= 'function setCurrentStores(){
            ...
            if (websiteStores[website]) {
              ...
              var option = document.createElement("option");
              option.appendChild(document.createTextNode(\''.Mage::helper('gri_giftcardaccount')->__('-- First Please Select a Website --').'\'));
