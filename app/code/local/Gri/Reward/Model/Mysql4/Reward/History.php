 ...
 */
class Gri_Reward_Model_Mysql4_Reward_History extends Mage_Core_Model_Mysql4_Abstract
{
    /**
    ...
    {
        $this->_init('gri_reward/reward_history', 'history_id');
    }
     ...
     * @return Gri_Reward_Model_Mysql4_Reward_History
     */
    ...
    public function isExistHistoryUpdate($customerId, $action, $websiteId, $entity)
    {
        $select = $this->_getWriteAdapter()->select()
            ->from(array('reward_table' => $this->getTable('gri_reward/reward')), array())
    ...
    public function getTotalQtyRewards($action, $customerId, $websiteId)
    {
        $select = $this->_getReadAdapter()->select()->from(array('history_table' => $this->getMainTable()), array('COUNT(*)'))
            ->joinInner(array('reward_table' => $this->getTable('gri_reward/reward')),
                'history_table.reward_id = reward_table.reward_id', array())
            ->where("history_table.action=?", $action)
    ...
    public function expirePoints($websiteId, $expiryType, $limit)
    {
        ...
        if (count($expiredHistoryIds) > 0) {
            ...
            foreach ($expiredAmounts as $rewardId => $expired) {
                ...
                $this->_getWriteAdapter()->update($this->getTable('gri_reward/reward'), $bind, $where);
            }
     ...
     * @return Gri_Reward_Model_Mysql4_Reward
     */
    public function updateHistoryRow(Gri_Reward_Model_Reward_History $object, $data)
    {
