 ...
 *
 */
class Gri_Logging_Model_Handler_Controllers
{
    ...
    */
     ...
     * @param Varien_Simplexml_Element $config
     * @param Gri_Logging_Model_Event $eventModel
     * @param Gri_Logging_Model_Processor $processor
     * @return Gri_Logging_Model_Event
     */
    public function postDispatchConfigSave($config, $eventModel, $processor)
    {
        ...
        $change = Mage::getModel('gri_logging/event_changes');
    ...
    public function postDispatchForgotPassword($config, $eventModel)
    {
        if (Mage::app()->getRequest()->isPost()) {
            if ($model = Mage::registry('gri_logging_saved_model_adminhtml_index_forgotpassword')) {
                $info = $model->getId();
            } else {
     ...
     * @param Varien_Simplexml_Element $config
     * @param Gri_Logging_Model_Event $eventModel
     * @param Gri_Logging_Model_Processor $processorModel
     * @return Gri_Logging_Model_Event
     */
    public function postDispatchPromoCatalogSaveAndApply($config, $eventModel, $processorModel)
    {
        ...
        if ($request->getParam('auto_apply')) {
            $eventModel->setInfo(Mage::helper('gri_logging')->__('%s & applied', $eventModel->getInfo()));
        }
    ...
    public function postDispatchTaxRatesImport($config, $eventModel)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('gri_logging')->__('Tax Rates Import'));
    }
    ...
    public function postDispatchProductUpdateAttributes($config, $eventModel, $processor)
    {
        ...
        return $eventModel->setInfo(Mage::helper('gri_logging')->__('Attributes Updated'));
    }
    ...
    {
        $request = Mage::app()->getRequest();
        $customersQty = Mage::getModel('gri_customersegment/segment')->getResource()
                ->getSegmentCustomersQty($request->getParam('id'));
        return $eventModel->setInfo(
            $request->getParam('id') ?
                Mage::helper('gri_customersegment')->__('Matched %d Customers of Segment %s', $customersQty, $request->getParam('id')) :
                '-'
        );
    }
    ...
    public function postDispatchSystemBackupsDelete($config, $eventModel)
    {
        ...
        if ($backup) {
            $eventModel->setIsSuccess($backup->getIsSuccess())
                ->setInfo(Mage::helper('gri_logging')->implodeValues($backup->getDeleteResult()));
        } else {
            $eventModel->setIsSuccess(false);
        }
        return $eventModel;
    ...
    public function postDispatchSystemCurrencySave($config, $eventModel, $processor)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('gri_logging')->__('Currency Rates Saved'));
    }
    ...
    public function postDispatchSaveCacheSettings($config, $eventModel, $processor)
    {
        ...
        if (is_array($cacheTypes) && !empty($cacheTypes)) {
            ...
            $info = Mage::helper('gri_logging')->__('Cache types: %s ', $cacheTypes);
        }
    ...
    public function postDispatchTaxRatesExport($config, $eventModel)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('gri_logging')->__('Tax Rates Export'));
    }
