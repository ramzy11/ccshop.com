 ...
 *
 */
class Gri_CustomerBalance_Model_Balance_History extends Mage_Core_Model_Abstract
{
    ...
    {
        return array(
            self::ACTION_CREATED  => Mage::helper('gri_customerbalance')->__('Created'),
            self::ACTION_UPDATED  => Mage::helper('gri_customerbalance')->__('Updated'),
            self::ACTION_USED     => Mage::helper('gri_customerbalance')->__('Used'),
            self::ACTION_REFUNDED => Mage::helper('gri_customerbalance')->__('Refunded'),
            self::ACTION_REVERTED => Mage::helper('gri_customerbalance')->__('Reverted'),
        );
    }
     ...
     *
     * @return Gri_CustomerBalance_Model_Balance_History
     */
    protected function _beforeSave()
    {
        $balance = $this->getBalanceModel();
        if ((!$balance) || !$balance->getId()) {
            Mage::throwException(Mage::helper('gri_customerbalance')->__('Balance history cannot be saved without existing balance.'));
        }
        ...
        switch ((int)$balance->getHistoryAction())
        {
                    ...
                    if ($user = Mage::getSingleton('admin/session')->getUser()) {
                        if ($user->getUsername()) {
                            if (!trim($balance->getComment())){
                                $this->setAdditionalInfo(Mage::helper('gri_customerbalance')->__('By admin: %s.', $user->getUsername()));
                            }else{
                                $this->setAdditionalInfo(Mage::helper('gri_customerbalance')->__('By admin: %1$s. (%2$s)', $user->getUsername(), $balance->getComment()));
                            }
                        }
                    }
                } else {
            ...
            case self::ACTION_USED:
                ...
                $this->setAdditionalInfo(Mage::helper('gri_customerbalance')->__('Order #%s', $balance->getOrder()->getIncrementId()));
                break;
            case self::ACTION_REFUNDED:
                $this->_checkBalanceModelOrder($balance);
                if ((!$balance->getCreditMemo()) || !$balance->getCreditMemo()->getIncrementId()) {
                    Mage::throwException(Mage::helper('gri_customerbalance')->__('There is no creditmemo set to balance model.'));
                }
                $this->setAdditionalInfo(Mage::helper('gri_customerbalance')->__('Order #%s, creditmemo #%s',
            ...
            case self::ACTION_REVERTED:
                ...
                break;
            default:
                Mage::throwException(Mage::helper('gri_customerbalance')->__('Unknown balance history action code'));
                // break intentionally omitted
        }
    ...
    protected function _afterSave()
    {
        ...
        if ($this->getBalanceModel()->getNotifyByEmail()) {
            ...
            $email->sendTransactional(
                Mage::getStoreConfig('customer/gri_customerbalance/email_template', $storeId),
                Mage::getStoreConfig('customer/gri_customerbalance/email_identity', $storeId),
    ...
    protected function _checkBalanceModelOrder($model)
    {
        if ((!$model->getOrder()) || !$model->getOrder()->getIncrementId()) {
            Mage::throwException(Mage::helper('gri_customerbalance')->__('There is no order set to balance model.'));
        }
    }
}
