 ...
 *
 */
class Gri_CustomerBalance_Model_Observer
{
    /**
     ...
     * @return  Gri_CustomerBalance_Model_Observer
     */
    protected function _checkStoreCreditBalance(Mage_Sales_Model_Order $order)
    {
        if ($order->getBaseCustomerBalanceAmount() > 0) {
            ...
            if (($order->getBaseCustomerBalanceAmount() - $balance) >= 0.0001) {
                ...
                Mage::throwException(Mage::helper('gri_customerbalance')->__('Not enough Store Credit Amount to complete this Order.'));
            }
        }
    ...
    public function processOrderPlace(Varien_Event_Observer $observer)
    {
        ...
        if ($order->getBaseCustomerBalanceAmount() > 0) {
            ...
            $websiteId = Mage::app()->getStore($order->getStoreId())->getWebsiteId();
            Mage::getModel('gri_customerbalance/balance')
                ->setCustomerId($order->getCustomerId())
                ->setWebsiteId($websiteId)
                ->setAmountDelta(-$order->getBaseCustomerBalanceAmount())
                ->setHistoryAction(Gri_CustomerBalance_Model_Balance_History::ACTION_USED)
                ->setOrder($order)
                ->save();
        }
    ...
    protected function _revertStoreCreditForOrder(Mage_Sales_Model_Order $order)
    {
        ...
        Mage::getModel('gri_customerbalance/balance')
            ...
            ->setAmountDelta($order->getBaseCustomerBalanceAmount())
            ->setHistoryAction(Gri_CustomerBalance_Model_Balance_History::ACTION_REVERTED)
            ->setOrder($order)
            ->save();
    ...
    {
        if (!Mage::helper('gri_customerbalance')->isEnabled()) {
            unset($observer->getUpdates()->gri_customerbalance);
        }
    }
    ...
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if (!$creditmemo->getRealRegister()) return $this;
        if ($creditmemo->getAutomaticallyCreated()) {
            if (Mage::helper('gri_customerbalance')->isAutoRefundEnabled()) {
        ...
        if ($creditmemo->getSalesHelper()->isCreditmemoTwoStep()) {
            if ($creditmemo->getCustomerBalanceReturnMax() === NULL) {
                $creditmemo->setCustomerBalanceReturnMax($creditmemo->getCustomerBalanceAmount() + $creditmemo->getGiftCardsAmount() + $creditmemo->getGrandTotal());
            }
            $creditmemo->setCustomerBalanceRefundFlag((boolean)$creditmemo->getCustomerBalanceTotalRefunded());
        }
        ...
        if ((float)(string)$creditmemo->getCustomerBalanceTotalRefunded() > (float)(string)$customerBalanceReturnMax) {
            Mage::throwException(Mage::helper('gri_customerbalance')->__('Store credit amount cannot exceed order amount.'));
        }
        //doing actual refund to customer balance if user have submitted refund form
        if ($creditmemo->getCustomerBalanceRefundFlag() && ($creditmemo->getBaseCustomerBalanceTotalRefunded() * 1)) {
            ...
            $balance = Mage::getModel('gri_customerbalance/balance')
                ...
                ->setHistoryAction(Gri_CustomerBalance_Model_Balance_History::ACTION_REFUNDED)
                ->setOrder($order)
    ...
    public function creditmemoDataImport(Varien_Event_Observer $observer)
    {
        ...
        if (isset($input['refund_customerbalance_return']) && isset($input['refund_customerbalance_return_enable'])) {
            ...
            if ($enable && is_numeric($amount)) {
                ...
                if ($amount) {
                    ...
                    $creditmemo->setBaseCustomerBalanceTotalRefunded($baseAmount);

                    // Fixed currency exchange precision issue
                    if ($baseAmount == $creditmemo->getBaseGrandTotal()) {
                        $amount = $creditmemo->getGrandTotal() * 1;
                    } else if ($baseAmount == $creditmemo->getBaseSubtotal()) {
                        $amount = $creditmemo->getSubtotal() * 1;
                    } else if ($baseAmount == $creditmemo->getBaseShippingAmount()) {
                        $amount = $creditmemo->getShippingAmount() * 1;
                    } else {
        ...
        } else if (Mage::registry('update_creditmemo')) {
    ...
    public function predispatchPrepareLogging($action) {
        ...
        if (isset($data['amount_delta']) && $data['amount_delta'] != '') {
            $actions = Mage::registry('gri_logged_actions');
            if (!is_array($actions)) {
                $actions = array($actions);
            }
            $actions[] = 'adminhtml_customerbalance_save';
            Mage::unregister('gri_logged_actions');
            Mage::register('gri_logged_actions', $actions);
        }
    }
    ...
    {
        Mage::getModel('gri_customerbalance/balance')->setCustomersBalanceCurrencyTo(
        ...
        if ($paypalCart) {
            ...
            if ($value > 0.0001) {
                $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, (float)$value,
                    Mage::helper('gri_customerbalance')->__('Store Credit (%s)', Mage::app()->getStore()->convertPrice($value, true, false))
                );
            }
        }
    }
