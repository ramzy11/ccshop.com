...
class Gri_GiftCard_Model_Observer extends Mage_Core_Model_Abstract
{
    ...
    public function setAmountsRendererInForm(Varien_Event_Observer $observer)
    {
        ...
        if ($elem) {
            $elem->setRenderer(Mage::app()->getLayout()->createBlock('gri_giftcard/adminhtml_renderer_amount'));
        }
    }
     ...
     *
     * @param Varien_Event_Observer $observer
     * @return Gri_GiftCard_Model_Observer
     */
    public function appendGiftcardAdditionalData(Varien_Event_Observer $observer)
    {
        ...
        if ($product->getUseConfigLifetime()) {
            $lifetime = Mage::getStoreConfig(Gri_GiftCard_Model_Giftcard::XML_PATH_LIFETIME, $orderItem->getStore());
        } else {
        ...
        if ($product->getUseConfigIsRedeemable()) {
            $isRedeemable = Mage::getStoreConfigFlag(Gri_GiftCard_Model_Giftcard::XML_PATH_IS_REDEEMABLE, $orderItem->getStore());
        } else {
        ...
        if ($product->getUseConfigEmailTemplate()) {
            $emailTemplate = Mage::getStoreConfig(Gri_GiftCard_Model_Giftcard::XML_PATH_EMAIL_TEMPLATE, $orderItem->getStore());
        } else {
    ...
    public function generateGiftCardAccounts(Varien_Event_Observer $observer)
    {
        ...
        $requiredStatus = Mage::getStoreConfig(Gri_GiftCard_Model_Giftcard::XML_PATH_ORDER_ITEM_STATUS, $order->getStore());

        foreach ($order->getAllItems() as $item) {
            if ($item->getProductType() == Gri_GiftCard_Model_Catalog_Product_Type_Giftcard::TYPE_GIFTCARD) {
                $qty = 0;
                ...
                if ($qty > 0) {
                    ...
                    $amount = Mage::getSingleton('gri_reward/reward')->convertGiftCardPointsToAmountByOrder($order);
                    ...
                    for ($i = 0; $i < $qty; $i++) {
                        try {
                            ...
                            Mage::dispatchEvent('gri_giftcardaccount_create', array('request'=>$data, 'code'=>$code));
                            $codes[] = $code->getCode();
                            $goodCodes++;
                    ...
                    if ($goodCodes && $item->getProductOptionByCode('giftcard_recipient_email')) {
                        ...
                        $codeList = Mage::helper('gri_giftcard')->getEmailGeneratedItemsBlock()
                            ->setCodes($codes)
                            ->setIsRedeemable($isRedeemable)
                        ...
                        $email->sendTransactional(
                            ...
                            Mage::getStoreConfig(Gri_GiftCard_Model_Giftcard::XML_PATH_EMAIL_IDENTITY, $item->getOrder()->getStoreId()),
                ...
                if ($hasFailedCodes) {
                    ...
                    $message = Mage::helper('gri_giftcard')->__('Some of Gift Card Accounts were not generated properly. You can create Gift Card Accounts manually <a href="%s">here</a>.', $url);
    ...
    public function loadAttributesAfterCollectionLoad(Varien_Event_Observer $observer)
    {
        ...
        foreach ($collection as $item) {
            if (Gri_GiftCard_Model_Catalog_Product_Type_Giftcard::TYPE_GIFTCARD == $item->getTypeId()) {
