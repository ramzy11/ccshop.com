...
class Gri_GiftCard_Block_Catalog_Product_View_Type_Giftcard extends Mage_Catalog_Block_Product_View_Abstract
{
    ...
    public function getGiftRedemptionUrl()
    {
        return $this->getUrl('gift-redemption');
    }
    ...
    public function getLoginUrl()
    {
        ...
        return $this->getUrl('customer/account/login', array(
            'back_url' => $urlHelper->getCurrentBase64Url(),
        ));
    }

    public function getRedeemUrl(Mage_Catalog_Model_Product $product)
    {
        return $this->getUrl('giftcard/product/redeem', array('id' => $product->getId()));
    }
    ...
    public function isMessageAvailable($product)
    {
        if ($product->getUseConfigAllowMessage()) {
            return Mage::getStoreConfigFlag(Gri_GiftCard_Model_Giftcard::XML_PATH_ALLOW_MESSAGE);
        } else {
    ...
    {
        return (int) Mage::getStoreConfig(Gri_GiftCard_Model_Giftcard::XML_PATH_MESSAGE_MAX_LENGTH);
    }
